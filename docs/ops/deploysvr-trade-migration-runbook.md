...
- **Last verified:** （待填）
- **停机窗口:** （待填；H5 教训：大文件走老→新直传，勿经本机 Mac 管道）
- **Live state:** （待填：新机 systemd 单元状态、release SHA、venv 版本）
- **Verified parity:** （待填：逐表行数）
- **DNS / Edge / TLS:** （待填：Cloudflare record、certbot 证书到期、nginx vhost）
- **CI/CD:** （待填：GitHub secrets 更新、push-to-deploy 实跑）
- **Rollback controls:** （待填）
