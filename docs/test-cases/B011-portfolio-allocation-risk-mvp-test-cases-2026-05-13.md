...
|---|---|---|---|---|
| TC-01 | Tooling baseline | Run full pytest suite. | All tests pass locally. | `./.venv/bin/python -m pytest` -> 155 passed. |
...
| TC-05 | Clean env safety | Run B011 Master Portfolio tests with an empty environment. | Master tests pass without env vars/secrets. | 64 B011 tests passed under `env -i`. |
...
| TC-08 | Quarterly cadence | Pass multiple signal dates from the same calendar quarter. | Master should consume only quarter-end weights or reject/filter intra-quarter dates. | FAIL: three same-quarter dates produced three rebalances. |
...
FAIL. TC-08 is a blocking acceptance gap.

## Required Fix Focus

Add an objective quarterly cadence boundary. Acceptable fixes include deriving quarter-end signal dates from fixture trading dates, filtering user-provided signal dates to the last trading day per calendar quarter, or failing closed when `run_master_portfolio_quarterly_backtest()` receives non-quarter-end or multiple same-quarter dates. Add regression tests proving the Master does not rebalance on intra-quarter child signals.
