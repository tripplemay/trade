...
| **C — Layer 0.5 完整交付（2026-06-07 重定义）** | Phase 3 UI（B037-B043）+ **真实评分基础（B044/B045/B046）** + **交易安全/风控/合规层真实化（B048 F011 根因）** + **用户交易闭环端到端可用（BL-B023-S1 生产冒烟）** + **HK-China 实现（BL-B011-S2，Master 4/4）** + **回测引擎真实化（B047：Backtest 去合成 stub + Reports 改真实业绩报告，去开发签收语料）** + **🎯 全页面真实化硬标准：所有用户投资页面显示内容均接真实引擎、无合成/占位（B049 穷举审计收口 gate；内部工具页 backlog/dev/docs 除外）** | 每日 Home 高频可用 + UI 简化 + **可按系统指示手动交易（含真实风控）** + 回测可信 + **全页面真实** | 进入 Phase 4 长尾按需 |
