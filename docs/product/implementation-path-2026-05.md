...
| **C — Layer 0.5 完整交付（2026-06-07 重定义）** | Phase 3 UI（B037-B043）+ **真实评分基础（B044/B045/B046）** + **用户交易闭环端到端可用（真实评分→mark-to-market diff→ticket→fills→reconcile→journal，BL-B023-S1 生产冒烟）** + **HK-China 实现（BL-B011-S2，Master 4/4 真实）** + **回测页接真实引擎（B047，去合成 stub）** | 每日 Home 高频可用 + UI 简化 + **可按系统指示手动交易** + 回测可信 | 进入 Phase 4 长尾按需 |
