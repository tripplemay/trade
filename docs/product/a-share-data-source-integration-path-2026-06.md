...
## 9. P1 符号/市场维度 Schema 设计（2026-06-13）

> **这是 A 股接入最横切的地基。** 设计目标：让 A 股符号与现有 US 符号**共存**，**不破现有 US 数据、不做全表迁移**，且把市场维度**限制在数据/lookup 层**（P1 不碰账户/NAV/执行/策略）。

### 9.1 核心决策：市场限定的 canonical 符号**字符串**（后缀约定）+ 派生 SymbolRef

**不做**「给所有表加 market/exchange 列的复合主键迁移」（系统有 N 张表都按裸 `symbol` 字符串键，复合键迁移=动全身）。
**做**：定义一个**市场限定的 canonical 符号字符串**作为唯一身份，US 裸 ticker 仍合法（默认 = US），CN 用后缀；结构化的 market/currency/board 由字符串**派生**（`SymbolRef` 值对象），不作处处存的必填列。

- **理由**：①最不侵入——存储层仍是一个 symbol 字符串列；②**向后兼容**——现有 `AAPL` 行不动（解释为 US）；③**无碰撞**——CN canonical 是 6 位数字+后缀，与 US 字母 ticker 不冲突，可同表共存。

### 9.2 canonical 约定 + provider 格式适配

| | canonical（我们存的）| 各 provider 原生格式（适配层转）|
|---|---|---|
| US | 裸 `AAPL`（默认市场=US）| yfinance `AAPL` |
| 沪 | `600519.SH` | baostock `sh.600519` / AkShare `600519`+市场参数 / Yahoo `600519.SS` |
| 深 | `000001.SZ` | baostock `sz.000001` / AkShare `000001` / Yahoo `000001.SZ` |

**关键**：每个数据源符号格式都不同 → **canonical ↔ native 适配是 symbology 层的核心职责**（挂在 B059 `SymbolDataProvider` 每个 provider 实现上）。

### 9.3 SymbolRef 值对象（派生 + 校验，单一来源）

```
SymbolRef.parse(canonical) -> {
  canonical: str          # 我们存储/传递的唯一身份，如 "600519.SH"
  code:      str          # "600519"
  market:    "US"|"CN"    # 由后缀推断（无后缀=US 默认）
  exchange:  "XNAS"|"XSHG"|"XSHE"|...   # MIC，可选
  currency:  "USD"|"CNY"  # 由 market 推断
  board:     "us"|"sh_main"|"sz_main"|"chinext"|"star"  # 由 code 前缀推断
}
```
- **board 由 code 前缀推断**（为未来交易规则预留，P1 不用）：`600/601/603/605→沪主板`、`688→科创`、`000/001/002/003→深主板`、`300/301→创业板`。
- **校验**：CN=6 位数字+`.SH/.SZ`；US=字母 ticker。**消歧坑**：`BRK.B`（US 类别股）的 `.B` 不是市场码 → 判为 US；只有后缀 ∈ 已知市场码集合（SH/SZ/…）才算市场限定。parser 须按"后缀是否市场码"区分，不能见点就拆。

### 9.4 向后兼容（铁律）

- **裸 ticker（无市场后缀）→ 解释为 US**（market=US/currency=USD）。现有全部 US 数据/键/路径**零改动、零迁移**。
- `normalize(symbol)`：裸→US 默认；带市场码后缀→解析市场。**所有读写经 normalize**，保证一致。

### 9.5 数据层 schema（最小增量）

- 价格缓存/快照（B059 本就新建缓存）：按 **canonical symbol 字符串**键 → CN 与 US 同表共存（canonical 不碰撞）。
- **可选**加 `market` + `currency` 两列（默认 `US`/`USD`，CN 填实），**仅为查询效率 + 显式**，非必需（可派生）。**不动**现有 US-only 的下游表（account/recommendation/paper/ticket）。

### 9.6 交易日历（按市场）

- `TradingCalendar(market)`：CN 日历（中国节假日，与 US 无重叠）。
- `quality.py` 的 `trading_calendar_gaps` gap 检查须**按市场**选日历（CN 符号用 CN 日历），否则 CN 数据会被误判一堆"缺口"。

### 9.7 币种（P1 仅元数据/展示，不入组合聚合）

- 价格记录带 currency（US=USD/CN=CNY）；**lookup 展示**正确符号与量纲（`¥1,680` vs `$150`，量=股/手）。
- **P1 不做跨市场组合聚合 / FX**——A 股在 P1 是**数据-only（lookup）**，不进任何账户/NAV/组合。币种聚合是"真持有 A 股"时（远期）的事。

### 9.8 provider 路由（复用 B059 抽象）

- `SymbolDataProvider` 注册**按市场路由**：canonical 的 `market` → 选 US providers（yfinance 等）或 CN provider（AkShare/baostock）。
- A 股符号自动路由到 CN provider；裸/US 路由到现有 US providers。**这就是 A 股"插进 B059 插座"的接缝。**

### 9.9 P1 scope 边界（防蔓延）

| 动 | 不动（P1）|
|---|---|
| 符号身份(canonical+SymbolRef)、symbology 适配、CN provider、价格缓存(+可选 market/currency 列)、CN 日历、B059 lookup 支持 A 股代码 | 账户/NAV/组合/执行/票据（仍 US-only，不引入币种聚合）、策略/回测、交易规则(T+1/涨跌停/手数/ST)、FX |

### 9.10 P1 feature 草拆（立项时细化）

1. **F-symbology**：`SymbolRef` + canonical 约定 + normalize + 校验/消歧 + 测试（US 裸默认 / CN 解析 / BRK.B 消歧 / board 推断）。
2. **F-cn-provider**：CN 价格 provider（AkShare/baostock，按 P0 结论选）插 B059 抽象 + canonical↔native 适配 + cost_guard 限流 + 缓存。
3. **F-cn-calendar**：CN 交易日历 + gap 检查按市场。
4. **F-lookup-cn**：B059 详情页支持 A 股代码（EOD/CNY/源 诚实标注；复用 B059 UI）。
5. **F-codex**：真机验 A 股 lookup（如 `600519.SH`→茅台价格/图）+ US 回归不破 + 边界 + signoff。

> **设计精髓**：用"market 限定的 canonical 字符串 + 派生 SymbolRef + 裸=US 默认"，把市场维度做成**加一层**而非**动全身**——US 数据零迁移、CN 同表共存、市场维度限制在数据/lookup 层，账户/执行/策略一概不碰。
