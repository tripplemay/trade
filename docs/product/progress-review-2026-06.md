...
8. ⏳ **回测引擎真实化（Backtest + Reports）**（2026-06-07 用户确认）：B047 → Backtest 去合成 stub + **Reports 从开发签收语料改为真实投资业绩报告**（同源真实引擎；架构撞 §12.10.2 需先定）
9. ✅ **🎯 全页面真实化（2026-06-07 用户拍板硬标准）**：里程碑 C 完成时**所有用户投资页面显示内容均接真实引擎，无合成/占位**。收口 gate=**B049**（穷举审计每页 + 修残余显示：snapshots 真进度 / strategies 元数据更正 / dashboard 死路由清理）。内部工具页（backlog/dev/docs）不算投资内容、排除。**2026-06-09 B049 F004 签收 PASS：8/8 页面全真实引擎、零合成/占位 → 🎯🎯 里程碑 C 正式达成。**

### 9.1 全页面真实化覆盖矩阵（✅ 2026-06-09 全绿，里程碑 C 达成）

| 页面 | 真实化批次 | 审计 |
|---|---|---|
| Home | B037 real NAV/sleeves | ✅ real |
| Recommendations | B044/B045（20 pos 真实评分）+ B046（current_weight）+ B048（gate kill-switch / wash-sale）| ✅ real |
| Reports | B047（真实投资业绩报告 canonical，开发签收语料已挪 dev 视图）| ✅ real |
| Risk | B048（F011 kill-switch/per-sleeve mark-to-market）+ B042（UI）| ✅ real |
| Strategies | B046 F002（注册表对齐 master）+ B049 F002（去陈旧 synthetic note + hk_china implemented）| ✅ real |
| Backtest | B047（真实 trade 引擎）+ B047-OPS2（data-range + 深回填，6pt/112trades）| ✅ real |
| Snapshots | B049 F001（真进度——读真实 on-disk data state，去合成 5 段动画）| ✅ real |
| Execution（diff/ticket/fills/reconcile/journal）| B023 + B046（mark-to-market diff）| ✅ real |
| Dashboard | B049 F003 死路由退役（`/api/dashboard`→404）| ✅ 退役 |
| backlog / dev / docs | 内部工具页，不算投资内容（排除）| — |
...
| 6 | **B047** 回测引擎真实化 | Backtest 去合成 stub + **Reports 改真实业绩报告**（同源 trade 引擎；撞 §12.10.2）| 回测+Reports 可信 |
| —（与 5-6 并行）| B043 | AI 解释层（依赖真实评分齐）| 「为什么」可信 |
| **7（收口 gate）** | **B049** 全页面真实化审计 | 穷举每页无合成/占位 + 修残余（snapshots 进度/strategies note/dashboard 死路由）| **🎯 里程碑 C 达成证据** |
