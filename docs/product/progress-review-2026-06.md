...
9. ⏳ **🎯 全页面真实化（2026-06-07 用户拍板硬标准）**：里程碑 C 完成时**所有用户投资页面显示内容均接真实引擎，无合成/占位**。收口 gate=**B049**（穷举审计每页 + 修残余显示：snapshots 真进度 / strategies 元数据更正 / dashboard 死路由清理）。内部工具页（backlog/dev/docs）不算投资内容、排除。

### 9.1 全页面真实化覆盖矩阵（里程碑 C 达成 = 全绿）

| 页面 | 真实化批次 |
|---|---|
| Home | ✅ done |
| Recommendations | B046（current_weight）+ B048（gate kill-switch / wash-sale）|
| Reports | ⏳ **B047**（**修正：现接 docs/test-reports/ 开发签收语料=错误类别**；改为真实投资业绩报告 Master/sleeve 回测，开发签收挪 dev 视图）|
| Risk | B048（F011 kill-switch/per-sleeve 真实化）+ B042（UI）|
| Strategies | B046 F002（注册表对齐 master）+ B049（更正陈旧 synthetic note）|
| Backtest | B047（接真实 trade 引擎，去合成 stub）|
| Snapshots | B049（真进度；数据已 B045 真）|
| Execution（diff/ticket/fills/reconcile/journal）| B023 + B046（mark-to-market diff）|
