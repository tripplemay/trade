...
5. ⏳ **用户交易闭环端到端可用**：真实目标 + 真实 current → 准确 diff → ticket → 手动下单 → fills → reconcile → journal，**用真实评分跑通生产冒烟**（BL-B023-S1）
6. ⏳ **HK-China 10% 实现**（2026-06-07 用户确认实现）：BL-B011-S2 → Master 4/4 真实
7. ⏳ **回测页接真实引擎**（2026-06-07 用户确认纳入）：B047 → /api/backtests/run 从合成 stub 换真实 trade 引擎，回测指标可信（架构需先定，撞 §12.10.2）
...
| 2 | **BL-B023-S1** 交易闭环冒烟 | 真实 diff → ticket → fills → reconcile 生产端到端验证 | **🔑 闭环可用确证** |
| 3 | **BL-B011-S2** HK-China 实现 | satellite 策略落地 → Master 4/4 真实 | Master 真实度 |
| 4 | B042 + **BL-B023-S2** | Risk Panel 微调 + kill-switch UI 演练 | 体验完整 |
| 5 | **B047** 回测页接真实引擎 | /api/backtests/run 合成 stub → trade 真实引擎（撞 §12.10.2，架构需先定）| 回测可信 |
