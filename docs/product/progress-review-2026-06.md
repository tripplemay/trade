...
6. 🔲 **HK-China 10% 决策**：实现 BL-B011-S2（4/4 真实）或正式接受「10% defensive 停泊」为设计
...
| 顺序 | 批次 | 作用 | 对交易闭环 |
|---|---|---|---|
| 1（进行中）| B045-OPS1 | trade wheel 部署可靠性（S4）| 基础设施稳定 |
| 2 | **B046** | regime reconcile + **account current_weight** | **🔑 让 diff 真实可交易** |
| 3 | **交易闭环冒烟**（BL-B023-S1）| 真实评分 → ticket → fills → reconcile 生产端到端验证 | **🔑 闭环可用确证** |
| 4 | B042 | Risk Panel 微调 | 体验完整 |
| 5 | B043 | AI 解释层（依赖 1-3 真实评分齐）| 「为什么」可信 |
| —  | HK-China 决策 | 实现 or 接受停泊 | Master 真实度 |
