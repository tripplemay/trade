...
## B021 first-time VM bootstrap (executed 2026-05-16 by Planner under user authorization)

Discovered after Codex F006 first reverify attempt: the B021 spec implicitly assumed VM-side infrastructure (systemd units, nginx vhost, certbot cert, env file, venv, dirs) already existed; only "incremental redeploy" path was specced. None of those were installed on the VM, blocking Codex's L2 verification regardless of code state.

Planner ran the missing first-time bootstrap. Files copied / installed / verified:

| What | Where | Notes |
|---|---|---|
| Filesystem layout | `/srv/workbench/releases/`, `/srv/workbench/`, `/opt/workbench/`, `/var/log/workbench/`, `/etc/workbench/`, `/var/www/letsencrypt/` | dirs owned by `deploy:deploy` where deploy needs write; `/etc/workbench/` chmod 750 root:root |
| Python 3.11 venv | `/opt/workbench/.venv/` | `apt install python3.11 python3.11-venv python3.11-distutils`; pip 26.1.1, setuptools 82.0.1; deploy:deploy ownership |
| systemd unit files | `/etc/systemd/system/workbench-{backend,frontend,backup}.service` + `workbench-backup.timer` | 4 units installed (state=disabled); `systemctl daemon-reload` done; not yet enabled |
| Backup scripts | `/opt/workbench/workbench-{backup,restore}.sh` | mode 755 root:root |
| Deploy scripts | `/opt/workbench/scripts/{deploy,healthcheck,rollback}.sh` | mode 755 root:root |
| nginx vhost | `/etc/nginx/sites-enabled/trade.guangai.ai.conf` (symlink to sites-available) | `nginx -t` passes after cert |
| Let's Encrypt cert | `/etc/letsencrypt/live/trade.guangai.ai/{fullchain,privkey}.pem` | webroot mode via `/var/www/letsencrypt`, non-interactive (existing certbot account); expires 2026-08-13; renewal auto via existing certbot timer |
| logrotate rule | `/etc/logrotate.d/workbench` | backup.log monthly + app/access weekly |
| uvicorn log config | `/etc/workbench/uvicorn-logging.json` | minimal default+access formatters → `/var/log/workbench/uvicorn-error.log` + `/var/log/workbench/access.log`, RotatingFileHandler 10 MB × 5 |
| **Production env file** | `/etc/workbench/workbench.env` | **chmod 640 root:deploy**, 716 bytes, 12 lines, 8 keys present, no PLACEHOLDER strings. Generated by `.github/workflows/bootstrap-env.yml` (workflow_dispatch one-shot, reads 4 sensitive secrets from GitHub vault + 4 non-sensitive literals); SCPed to `deploy@:~/.bootstrap/workbench.env`; Planner (tripplezhou full sudo) ran `sudo install -m 640 -o root -g deploy ...` to place it at final path. tripplezhou cannot read 640 file (proves group-only read). Staging file deleted. |

### Outstanding work (user-action remaining)

1. **VM service account scope expansion** (B021 F005 prereq, not part of original 5 prep items): `devstorage.read_only` → `cloud-platform`. ~30-60s cross-service downtime for kolquest.com + staging.kolmatrix + apify-kol + pm2 aigcgateway. Run during low-traffic window:
   ```bash
   ...
   ```
2. After scope expansion, Planner will trigger first code deploy via `workbench-deploy.yml` (push a trivial commit to workbench/backend to trigger the CI chain). Code SCPs to `/srv/workbench/releases/$SHA/`, deploy.sh runs `alembic upgrade head` against the now-real env, symlink flips to current, services restart with real OAuth values, healthcheck passes.
3. Codex F006 then resumes L2 reverification (6 items: browser OAuth happy path, non-allowlist reject, /api/health version=HEAD, systemctl quota verification, GCS backup file present, neighbor services unaffected).

### Security notes for the env-file bootstrap path

- 4 sensitive values (GOOGLE_OAUTH_CLIENT_ID, GOOGLE_OAUTH_CLIENT_SECRET, NEXTAUTH_SECRET, ALLOWED_USER_EMAIL) live only in GitHub Secrets + the encrypted env file on the VM. No value transited via chat, CLI args, shell history, or git diff.
- The `bootstrap-env.yml` workflow runs in a GitHub-hosted runner (ephemeral container) and SSHes via the `DEPLOY_SSH_PRIVATE_KEY` secret. All secret references via `${{ secrets.* }}` are auto-masked in run logs.
