...
> ## ⛔ 裁定已被更正 — 本文 F004 裁定作废
>
> **最终裁定：`NO-GO`**（用户 2026-07-21 裁定）。本文原裁定
> `INCONCLUSIVE_COVERAGE_LIMITED` **不再有效**。
>
> 更正依据与完整论证见 **[`B110-F004-adjudication-correction-2026-07-21.md`](./B110-F004-adjudication-correction-2026-07-21.md)**：
> D1 在本案机械上不改变裁定（主口径两个基准读法都 ≤1.0%）／NO-GO 的第二个析取项
> （Q4>Q5，四口径全成立）对 D1/D6 免疫／**冻结附录 D8 已就此争议锁死「照字面执行、
> 不因统计不显著而放宽」，本文从未引用**。
>
> ★**本文正文保持原样不改写**，作为 F004 的验收证据链保全。事实层经独立复核基本无误；
> 被更正的是裁定层，以及三处证据强度问题（见更正文 §3）。
