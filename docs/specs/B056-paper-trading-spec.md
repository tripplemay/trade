...
**UI 布局（用户 2026-06-11 确认，6 区块；复用现有组件保持视觉一致）：**

| 区块 | 内容 | 复用 |
|---|---|---|
| **① 顶部摘要** | 策略选择器（多策略下拉切换，Master 先；B055 上线自动出现）+ 虚拟本金 + 激活日 + 运行天数 + **当前净值 + 总收益($+%) + 今日盈亏 + vs SPY（跑赢/跑输 %）** | 仿首页 NAV 卡 |
| **② 净值曲线** | 前向 NAV 时间序列（自激活日累积）+ **SPY 基准叠加线** | 仿回测页 equity 图 |
| **③ 每资产盈亏表** | 每标的：持仓股数/成本价/现价/市值/**浮盈亏($ 和 %，红绿)**/权重 + 现金行 | AG-Grid/DataTable |
| **④ 配置 vs 目标漂移** | 模拟盘当前权重 vs 策略最新目标的漂移（调仓日拉回提示）| 仿推荐页 AllocationBar |
| **⑤ 调仓记录（简版）** | 历次虚拟调仓**仅记日期 + 当次成本** + 累计成本 + 下次调仓日预告（**不记每笔买卖明细**）| 列表 |
| **⑥ 设置** | 激活模拟盘 / 设虚拟本金 / 选策略 | 表单 |

**用户确认决策：** ①基准=**SPY**；②**保留**配置 vs 目标漂移块（④）；③调仓记录=**简版**（日期+成本，不要每笔明细）。

1. **新页「模拟盘」**（per 策略，Master 先，顶部下拉切换）按上表 6 区块实现。
2. **中文**（B054 后，全中文；i18n parity）；明确标「模拟非实盘」。
3. **空/前向态**：刚激活单点→友好「模拟盘运行 X 天，前向累积中」。
4. **测试**：vitest（摘要/曲线+SPY/盈亏表/漂移块/简版调仓记录/空态）+ i18n parity + api.ts。
...
- 虚拟本金（用户设，默认如 $100k）；~~benchmark~~（**已定 SPY，2026-06-11**）；成本模型 bps 默认值；是否允许可选 seed-start-date（默认否，纯前向）；MTM timer 时刻（对齐 prices timer 后）。
- **UI 已定（2026-06-11，§6）**：6 区块 / SPY 基准 / 保留配置漂移块 / 调仓记录简版。
