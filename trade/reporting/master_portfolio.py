...
def render_master_portfolio_markdown(report: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            f"# Master Portfolio Report {run['run_id']}",
            "",
            "## Summary",
            f"- Portfolio: {strategy['portfolio_id']}",
            ...
            f"- High-water mark: {account_risk['high_water_mark']}",
            f"- Kill-switch active: {account_risk['kill_switch_active']}",
            f"- Kill-switch triggered at: {account_risk['kill_switch_triggered_at']}",
            f"- Human review required: {account_risk['human_review_required']}",
            "",
            "## Baseline Comparison",
            f"- Baseline label: {baseline['label']}",
            f"- Baseline ending value: {baseline['ending_value']}",
            f"- Followups absorbed: {baseline['followups_absorbed']}",
            "",
