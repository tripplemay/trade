...
def render_master_portfolio_markdown(report: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            f"# Master Portfolio Report / 旗舰组合回测报告 {run['run_id']}",
            "",
            "## Summary / 摘要",
            f"- Portfolio / 组合: {strategy['portfolio_id']}",
            ...
            "",
            "## Account Risk Controls / 账户风险控制",
            f"- Drawdown threshold / 回撤阈值: {account_risk['drawdown_threshold']}",
            f"- Current drawdown / 当前回撤: {account_risk['drawdown']}",
            f"- High-water mark / 历史高点: {account_risk['high_water_mark']}",
            f"- Kill-switch active / 熔断启用: {account_risk['kill_switch_active']}",
            f"- Kill-switch triggered at / 熔断触发于: {account_risk['kill_switch_triggered_at']}",
            f"- Human review required / 需人工复核: {account_risk['human_review_required']}",
            "",
            "## Baseline Comparison / 基准对比",
            f"- Baseline label / 基准标签: {baseline['label']}",
            f"- Baseline ending value / 基准期末价值: {baseline['ending_value']}",
            f"- Followups absorbed / 已吸收的跟进: {baseline['followups_absorbed']}",
            "",
