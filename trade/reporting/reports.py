...
def render_markdown_report(report: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"- 期末价值: {portfolio['ending_value']}",
            f"- 年化收益率: {metrics['CAGR']}",
            "",
            "## 数据与参数",
            f"- 数据快照: {data['data_snapshot_id']}",
            f"- 快照类型: {data['snapshot_kind']}",
            f"- 快照清单: {data['snapshot_manifest']}",
            f"- 参数哈希: {parameters['parameter_hash']}",
            f"- 动量窗口: {parameters['momentum_windows']}",
            "",
            "## 信号与执行",
            f"- 信号时点: {strategy['signal_timing']}",
            f"- 信号价格字段: {execution['signal_price_field']}",
            f"- 执行时点: {strategy['execution_timing']}",
            f"- 执行价格字段: {execution['execution_price_field']}",
            f"- 执行假设: {execution['execution_assumption']}",
            f"- 缺失 T+1 开盘价处理: {execution['missing_t_plus_1_open_policy']}",
            "",
            "## 业绩指标",
            ...
            "",
            "## 可复现性",
            f"- 快照校验和: {data['checksum']}",
            f"- 配置引用: {run['config_reference']}",
        ]
    )
