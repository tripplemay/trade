...
def render_markdown_report(report: dict[str, object]) -> str:
    ...
    return "\n".join(
        [
            f"# Backtest Report / 回测报告 {run['run_id']}",
            "",
            "## Summary / 摘要",
            f"- Strategy / 策略: {strategy['strategy_id']}",
            f"- Ending value / 期末价值: {portfolio['ending_value']}",
            f"- CAGR: {metrics['CAGR']}",
            "",
            "## Data And Parameters / 数据与参数",
            f"- Data snapshot / 数据快照: {data['data_snapshot_id']}",
            f"- Snapshot kind / 快照类型: {data['snapshot_kind']}",
            f"- Snapshot manifest / 快照清单: {data['snapshot_manifest']}",
            f"- Parameter hash / 参数哈希: {parameters['parameter_hash']}",
            f"- Momentum windows / 动量窗口: {parameters['momentum_windows']}",
            "",
            "## Signal And Execution / 信号与执行",
            f"- Signal timing / 信号时点: {strategy['signal_timing']}",
            f"- Signal price field / 信号价格字段: {execution['signal_price_field']}",
            f"- Execution timing / 执行时点: {strategy['execution_timing']}",
            f"- Execution price field / 执行价格字段: {execution['execution_price_field']}",
            f"- Execution assumption / 执行假设: {execution['execution_assumption']}",
            f"- Missing T+1 open policy / 缺失 T+1 开盘价处理: "
            f"{execution['missing_t_plus_1_open_policy']}",
            "",
            "## Performance Metrics / 业绩指标",
            f"- Volatility / 波动率: {metrics['annualized_volatility']}",
            ...
            f"- Snapshot checksum / 快照校验和: {data['checksum']}",
            f"- Config reference / 配置引用: {run['config_reference']}",
        ]
    )
