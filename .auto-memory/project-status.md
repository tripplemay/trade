...
---
## 当前状态
- **B041-recommendations-robinhood：`building`**（2026-06-06 启动；Phase 3 / Stream 5.B，**前端-only**）。target positions table → 简化 card + 专业 view toggle（Radix Tabs）。**核查关键发现**：『为什么这样建议』真实数据不存在——推荐引擎 equal-weight 占位（F011 真实评分未建），`rationale` 占位文本，无 quant signal 可引；富 AI『为什么』是 **B043**（依赖真实评分）。**决策（★用户批）：UI 重构 now，富『为什么』留 B043**；后端无改动（占位 rationale as-is）；颜色复用 B040 colorForMetric（delta 正绿 buy/负红 sell/0 中性）。复用 B040 MetricsDisplay/colorForMetric/双语 tooltip + Home sleeve 卡片 + Radix Tabs。2 features：(F001 g) PositionCards 简化 card+专业 toggle+复用 B040 原语+surface 占位 rationale+i18n+no-execution 守门+vitest+Playwright（仅切 target-positions 呈现，charts/gate/wash-sale/export-to-ticket/NewsPanel 不动）；(F002 c) L1+L2 两态手验+工作流不破+signoff（§23/§24 N/A）。spec `docs/specs/B041-recommendations-robinhood-spec.md`。
