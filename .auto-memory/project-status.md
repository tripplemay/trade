...
---
## 当前状态
- **B045-real-data-refresh-pipeline：`verifying`**（2026-06-07；**真实数据刷新 pipeline**，B044 Batch 2 数据线）。**F001+F002+F003（全 generator）已完成，CI 三次均绿，交 Codex F004 验收。** F001(e76a9af) `workbench_api/data_refresh` 刷新 CLI(Tiingo prices+SEC EDGAR fundamentals→`<data_root>/snapshots/{prices,fundamentals}/unified/*`)+`workbench-data-refresh.{service,timer}` 每日02:30 B037-OPS1 自动接线+scope 守门。F002(6817427) `trade/data/data_root.py` WORKBENCH_DATA_ROOT 覆盖(未设→repo-root 不变)+loader/us_quality resolver+`workbench-recommendations.service` 设 env。F003(90e5280) precompute `data_source` 粒度 real/mixed/fixture(honesty guard：unified 文件 .exists() 才标 real)+真实日频齐时 risk_parity 不再 stub(S3 消除)，不动 trade 评分逻辑，不破 snapshot 契约。**F004 交 Codex**：L1 守门+L2 真 VM(timer 自动接线/手动 trigger 刷新写真实 unified/precompute data_source=real|mixed 全 sleeve 非 stub/`/current` 真实权重 vs B044 fixture/disk 接 S1)+signoff。**风险**：SEC EDGAR fundamentals 覆盖不全→us_quality 可能 mixed(诚实) + disk(B044 S1 82%)。后续 **B046=regime reconcile+account current_weight**。spec `docs/specs/B045-real-data-refresh-pipeline-spec.md`。
