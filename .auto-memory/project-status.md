...
- **B048-OPS1-alembic-deploy-reliability：`building`**（2026-06-07 启动；横切 ops 修复，order 3）。修 B048 Finding #1——deploy 后 prod alembic 停 0006 缺 0007-0011（price_history 等表不存在需手动 upgrade）=真实部署缺陷（影响所有后续批次）。决策（★用户批）：拆出修部署（同 B045-OPS1）+ deploy 后断言 alembic==head 硬失败。根因候选=ENV_FILE 未加载→alembic 跑 scratch DB(B022 同款坑)。2 features：(F001 g)诊断+修+断言；(F002 c)L2 fresh deploy alembic==head 无需手动+表存在+signoff(§Finding #1/S2 resolved)。spec `docs/specs/B048-OPS1-alembic-deploy-reliability-spec.md`。
