...
## 生产状态
- `https://trade.guangai.ai` live；production `/api/health.version` = `ec0289495eaf10255c20064982ed33d554c5905b`（B034 F004 fix-round 1 已部署，Workbench Deploy success 2026-06-04；产品代码与 `main` HEAD 等价）；`WORKBENCH_DB_URL=sqlite:///var/lib/workbench/db/workbench.db`；`alembic_version=0006_b034_news_embedding`，`news` + `news_embedding` 表已存在；`/var/lib/workbench/data/snapshots/news` 已创建且为空；无 news scheduler.py / cron / systemd unit；**`/api/recommendations/news` 500 blocker 已修复并部署**（universe materialise 入代码常量，零文件 I/O）；anon `/news`=401（auth gate ok）；待 Codex L2 authenticated 复验返 200。
