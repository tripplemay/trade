...
---
## 当前状态
- **B047-OPS1-backtest-deploy-env-reliability：🔎 `verifying`**（2026-06-08；F001 done commit 8a3e325 CI 绿；OPS 部署可靠性，正交 B047 功能码；2 features 1g+1c）。**F001（generator）done**：(1) 新建 `db/require_production_db.py`——canonical/worker main 缺 env（WORKBENCH_DB_URL 回落 DEFAULT_DEV_DB_URL scratch）→`::error::` 非零退出不写 scratch；放行=显式 URL 或 `WORKBENCH_ALLOW_DEV_DB=1`；(2) deploy.sh post-step 断言 worker is-active + canonical timer is-enabled（warning 非 hard-fail，§12.11）；(3) schema check required 加 backtest_run+investment_report。Gates：pytest 940（+14）/ruff/mypy/bash -n 全绿。**F002（codex，verifying）** L2 见 features.json——★canonical 经 systemd 写 prod DB→/api/reports≥1→/reports 真实报告=B047 Reports 端到端闭合。**⚠️ 发现：Workbench Deploy workflow 实为 workflow_run 自动触发（green main CI 后 ssh/rsync VM），非纯用户手动**——与 spec/planner『F002 依赖用户手动触发』假设不符，F001 push 后 deploy 已自动跑；F002 prereq 自动满足。**✅ gap B 已闭合（用户授权 generator 直接 SSH VM）**：VM 应用新 sudoers（+3 worker grant），`visudo -c` 全树 OK；deploy 用户现可 restart worker；worker 已重启上 8a3e325/F001 代码，journal 无 guard error；真机正向验 bare canonical 无 env→`::error::` exit1 无 scratch 写。F001 真机闭环完成，F002 待 Codex（canonical service 经 systemd→/api/reports→/reports）。spec `docs/specs/B047-OPS1-backtest-deploy-env-reliability-spec.md`。
...
- **🎯 里程碑 C 路径（『所有页面接真实引擎』硬标准）**：B046(✅)→B048(✅)→B048-OPS1(✅)→BL-B023-S1(✅ §6 交易闭环)→HK-China(✅ Master 4/4)→B042 Risk UI(✅6)→B047 Backtest 真实✅/Reports 端到端待 **B047-OPS1(building)** 闭合(7)→**B049 全页面真实化审计 gate(8)**；B043 AI 解释并行。**里程碑 C §6 交易闭环+Master 4/4+Risk UI+Backtest 真实 均达成；余 Reports 端到端(B047-OPS1)+全页面审计(B049)+AI 解释(B043)。**详见 `docs/product/progress-review-2026-06.md` 覆盖矩阵。
