...
---
## 当前状态
- **B039-home-advisor-disclaimer：`verifying`**（2026-06-06 Generator F001 完成，Frontend CI success，待 Codex F002 L2）。Phase 3 / Stream 4.C **最小范围前端-only**：B037 复用 AdvisorSection 已渲染 advice/引用/INSUFFICIENT/per-sleeve，唯一 gap=mockup §2 disclaimer。**F001 交付**：AdvisorSection.tsx 加 card 级 `advisor-disclaimer`（**ok+insufficient+empty 三态常驻**，loading/error 隐藏）+ i18n `home.advisor.disclaimer` 双语 + 守门（messages-key-parity 显式永存断言 + no-execution 扩 AdvisorSection + AdvisorSection.spec +6 + b037-home Daily Journey advisor-disclaimer 双语可见）。**纯前端无 backend diff**。Gates：frontend lint0/typecheck/vitest 219(+10)/Playwright 绿。**关键设计**：disclaimer empty 态也显示，故 CI/prod advisor 空时仍可见（Playwright 不 flake）。**F002 Codex L2**：health 200+SHA≡main+recent-errors=0 + Home advisor 段 disclaimer 浏览器手验（双语+无下单按钮）+截图 + signoff；**纯前端无新路由/timer，§23/§24 标 N/A**。spec `docs/specs/B039-home-advisor-disclaimer-spec.md`。
