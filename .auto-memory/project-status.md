...
---
## 当前状态
- **B037-home-restructure：`verifying`**（generator F001-F003 全完成+CI 全绿+deploy 成功 77c50fa，2026-06-05；Phase 3 / Stream 4.A 首批 → 里程碑 C / Layer 0.5）。待 Codex F004 L1+L2 验收。**F001 后端 done**：`price_snapshot` 表(alembic 0009)+repo+`workbench-prices` CLI/timer(复用 B027 Tiingo loader 抓持仓 close)+positions `sleeve` 标签+PriceProvider+`build_home`(NAV/Day P&L mark-to-market/sleeve breakdown)+`GET /api/home`(auth-gated)+§12.10 守门+scheduler scope(r)含 prices；backend pytest 757(735+22)/ruff0/mypy0/alembic head 0009。**F002 前端 done**：三段 Home 替换旧 quant dashboard（① NAV+Day P&L hero ② 复用 AdvisorSection ③ 复用 MarketContextCard+sleeve breakdown）+双语+no-execution+vitest 198(40 files)。**F003 done**：§16 旧 Home 退役四处清理(死 i18n keys 清+`legacy-home-decommissioned` guard+no-execution 扩 Home)+Daily Journey e2e(`b037-home`，双 locale)+§22 presence→absence。spec `docs/specs/B037-home-restructure-spec.md`(§2.1 含 building 期 3 处设计澄清)。
