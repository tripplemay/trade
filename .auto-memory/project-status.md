...
---
## 当前状态
- **B040-reports-robinhood：`building`**（2026-06-06 启动；Phase 3 / Stream 5.A，**真实新批次非复用 gap**）。把回测指标从 quant-jargon 表格重构为 Robinhood-style：大数字+颜色编码+双语 tooltip（术语英文 Sharpe/Sortino/Calmar/MDD/CAGR + 中文 tooltip 解释）。**决策（★用户批）：两个面都做**——①`/backtest`（结构化 BacktestMetrics 现成，前端为主）+②`/reports/[slug]`（后端从异构 Markdown 指标表解析 graceful null + 前端大数字卡，**不破 body_markdown**）。planner 定：tooltip 确定性手写(AI 文案是 B043)/Calmar 派生 cagr/|mdd|/reports 解析=header-signature+同义词+graceful null(防脆弱 v0.9.21 对真实语料验证)/不新增 per-sleeve 路由。**定位 §1.1 硬边界：大数字=历史回测统计不出预期收益数字；tooltip 非 AI**。复用 radix Tooltip+/backtest MetricsCard 模板+i18n+MarkdownRenderer。**B040：`verifying`**（Generator F001+F002+F003 全完成，CI 全绿，待 Codex F004）。**F001 后端**：ReportMetrics(7 字段)+_parse_metrics(header-signature 宽表+同义词+Calmar 派生+graceful null)+body_markdown 字节不变+14 pytest(backend 808)。**F002 前端原语**：lib/metric-color.ts colorForMetric(阈值 spec §4.2)+components/metrics/MetricsDisplay.tsx(大数字+英文术语+颜色+radix 双语 tooltip)+i18n metrics.tooltips.7 指标(手写强调历史统计非预测)+接入 /backtest+vitest。**F003 /reports**：/reports/[slug] metrics 非空渲染卡上方/null graceful+body_markdown 不动+e2e b040-metrics 两面双 locale（含 Sharpe tooltip hover 双语）。**坑**：e2e Playwright tooltip 首轮失败=hover wrapper 非 radix trigger→给 TooltipTrigger 加 metric-label-<key> testid 精确 hover 修复（commit e9da1ef）。Gates backend 808/ruff0/mypy0；frontend lint0/typecheck/vitest 233/Playwright 绿。**F004 Codex L2**：/api/reports/{slug} 200 含 metrics 字段+两面浏览器手验(大数字/颜色/中文 tooltip/Markdown 保全/无下单/双语/§1.1 无预期收益)+截图≥2+signoff。**纯前端+后端解析无全新对外路由，§23/§24 N/A**。spec `docs/specs/B040-reports-robinhood-spec.md`。
