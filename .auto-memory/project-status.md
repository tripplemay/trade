...
- **B090 🔨 building（2026-07-05 开批, F001 scoped+attempted+checkpointed）** hk_china 真数据重测(修方法学 200D warmup + 真个股vs proxy)。★attempt 深入验证: 数据 OHLC 全本地可得(26 HK stock_hk_daily sina/4 proxy+SGOV stock_us_daily/FRED FX DEXHKUS public CSV, 非VM-gated)+精确接口全勘(unified schema/build_usd_frames/200D warmup=signal filter不重写harness)。★**撞到真 decision-grade 限制: akshare 无真 adj_close**(comparison 收益依赖; 需 Tiingo/VM 真adj_close 或 qfq近似+caveat 的 judgment call)→完整重测宜 fresh 增量测试建(handoff 有精确 scope+子问题)。research-only。
- **接续**：backlog 4 项(A股聪明钱[¥200 Tushare待用户]/test-automation[P3-P5生产/独立性]/B055[US careful strategy]/residual-engine[触冻结flagship待用户])。B090 execute-ready fresh。B081-B085 严验 follow-up 待并入。33 learnings 待用户确认。★下阶段战略决策待用户(P0-P2无强edge; 安全可测机械win已尽 B086-B089)。
