...
---
## 当前状态
- **B038-home-market-news：`fixing`**（2026-06-06 Codex F003 首轮 L2 blocker）。Phase 3 / Stream 4.B = 把今日市场新闻搬上 Home 第三段(personas §2 mockup gap；指标卡 B037 已复用 MarketContextCard 完成)。**L1 已绿**：backend targeted pytest `43 passed`（`/api/news/latest` + scope/non-generative/news_no_scheduler/self-contained 守门）+ frontend targeted vitest `28 passed`（HomeNewsPanel + no-execution）。**L2 已通过部分**：prod `/api/news/latest` authenticated `200 {"items":[]}`、anon `401`、authenticated `/api/debug/recent-errors={"count":0,"records":[]}`；`workbench-news.timer` **已由 B037-OPS1 durable 路径自动接线**（`enabled + active(waiting)`，deploy run `27052426615` 明示 `install + enable workbench-news.timer` + `✓ enabled`，无 wiring warn）。**当前 blocker**：手动 `workbench-news.service` oneshot 失败，journal 报 `ModuleNotFoundError: No module named 'scripts.universe_us_quality'`，源于 `workbench_api/news/cli.py:_default_universe()` 运行时 import repo-root `scripts.universe_us_quality`。这属于 **v0.9.32 §12.10 deploy-artifact 自包含缺陷**：service/CLI 运行路径依赖 release 外 repo-root `scripts/`。blocker 报告：`docs/test-reports/B038-home-market-news-blocker-2026-06-06.md`。Generator 下一步需把 default universe materialize 进 `workbench_api/` 包内，再重 deploy 交 Codex 复验。
