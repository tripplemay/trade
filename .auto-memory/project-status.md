...
---
## 当前状态
- **B033-news-ingest：`reverifying`**（2026-06-01；fix-round 1 已交回 Codex 复验）。F004 L2 blocker（production 缺 data/snapshots/news/）已修：commit 77a455a — deploy.sh 在每次部署 mkdir -p 持久路径 `/var/lib/workbench/data/snapshots/news`（与 SQLite DB 同根，存活 release GC/swap）+ symlink release 相对路径到该持久目录；建空不跑 ingest（边界 (q)）；回归守门 `tests/safety/test_news_snapshot_dir_provisioned.py`。✅ Deploy 已完成（Workbench Deploy run 26730625040 / headSha 94a038b / success + healthcheck 过），新 deploy.sh 已在 VM 执行 mkdir；Codex 可直接复验。F001/F002/F003 generator ✅ done；F004 L1 全 PASS（backend pytest 572 passed+2 skipped / ruff 0 / mypy 0 / alembic 0005↔0004 / frontend lint+typecheck / vitest 172 / Playwright 38）。blocker 报告 `docs/test-reports/B033-news-ingest-blocker-2026-06-01.md`。遗留(非阻塞)：CLI DEFAULT_SNAPSHOT_ROOT 生产解析不准，B034 首跑 ingest 前须接 `WORKBENCH_NEWS_SNAPSHOT_DIR`。
