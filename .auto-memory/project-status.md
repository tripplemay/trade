...
---
## 当前状态
- **B014-regime-adaptive-stress-validation：`fixing`**；Codex 已确认真实 Stooq CSV 路径被 apikey 门槛阻塞，现需 Generator 修复 `scripts/fetch_stooq_regime_adaptive_csvs.py` 的 apikey 支持后再继续 F003-F006。
...
- Codex 反馈：直接访问 `https://stooq.com/q/d/l/?s=spy.us&i=d&d1=20180101&d2=20251231` 返回 apikey 门户页，不给 CSV；本地 fetcher/tests 绿色，但真实抓取无法闭环。
...
- B014 真实 snapshot 抓取被 Stooq apikey 门槛阻塞；需 generator 修复 fetcher 以支持显式 apikey 输入，且仍保持 stooq.com-only / stdlib-only 约束。
