...
---
## 当前状态
- **B047-OPS1-backtest-deploy-env-reliability：🔨 `building`**（2026-06-08 planning done；OPS 部署可靠性，正交 B047 功能码，同 B048-OPS1 先例；2 features 1g+1c）。修 B047 backtest/Reports 的部署+env gap，闭合 Reports 真机端到端。三 gap：(A) canonical/worker CLI 缺 env→get_engine() 静默回落 DEFAULT_DEV_DB_URL scratch DB（re-verify 裸跑 canonical 写 scratch、API 读 prod→/api/reports 0，被误诊『读路径 bug』实为 env 写错 DB）；(B) worker 初始 disabled（sudoers 一次性应用 S1）；(C) alembic 0012-0013 未自动升/部署链是否真跑 B047（部署用户手动触发）。F001 g：canonical/worker main 缺 env 硬失败不写 scratch（B048-OPS1 env 守门套到新 CLI 入口）+ deploy.sh worker is-active/canonical timer is-enabled post-step 断言+required-tables 含两表+守门测试；F002 c：**依赖用户触发 deploy 后** L2——alembic heads+两表/worker auto-active(非手动)/canonical timer/★canonical 经 systemd 写 prod DB→/api/reports≥1→/reports 渲染真实报告(B047 Reports 端到端闭合)/CLI 缺 env 响亮失败正向验。**F002 强依赖用户手动触发部署。** spec `docs/specs/B047-OPS1-backtest-deploy-env-reliability-spec.md`。
