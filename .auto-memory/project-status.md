...
## 当前状态
- **当前：B074 verifying（生产 hotfix，铁律 9；F001+F002 generator done，交 Codex F003 真机）** = cn_attack A股 模拟盘建仓修复。**★双根因**:#1(planner VM 实测)price_snapshot 0 个 A股(25 目标 0 可估价)——A股 价在统一 CSV 但没进 price_snapshot(prices/cli Tiingo US-only + price_universe 全美股);**#2(generator 实测发现,planner 未覆盖)** cn_attack target 含字面 CASH 缓冲行(无 mark)被 pure engine 计入 skipped_symbols→build_complete 永 False(即便 A股 价已同步)。两根因都修才到 build_complete=1。**修**:F001=新模块 cn_snapshot_sync 从统一 CSV 同步 A股 收盘进 price_snapshot(SymbolRef 识 A股,不碰 Tiingo/price_universe,零回归);F002=paper/targets 剥离 CASH_SENTINEL(只影响 cn_attack;Master/regime 用实 ETF SGOV 零回归)+acceptance 守门(active paper 目标可估价永不复发,含 teeth 测试)。本机门禁全绿(ruff/mypy + 1583 passed)。F003=Codex L2 真机(两账户 build_complete=1+持仓+**cash≈cash_buffer 非≈0**,spec『cash≈0』沿用 Master 模板对 cn_attack 不准)。研究态 paper(无真金),不改 cn_attack 策略本身。
