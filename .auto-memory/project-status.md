...
## 当前状态
- **当前：B075 building** = A股 生产股票池扩大到全市场流动 top ~1500(feasibility-first,用户选最广档)。**★命门=数据负载**:价格逐只 akshare(1500 次/天)重;基本面季度才变→低频刷(周/月);§23 bulk 发现 stock_zh_a_spot_em 走 eastmoney push 主机 dev box SSL-fail→可行性必须 VM 实测。**feasibility-gated**:目标 1500 但 F001 VM 实测定可行 N,刷不动诚实退最大可行 N(不静默 cap,B070 先例)。3 features 2g+1c:F001 宽 universe 机制(ungate)+VM 可行性闸/F002 接生产数据刷新+price_snapshot 同步(复用 B074)+cn_attack 宽池选股/F003 Codex L2 真机+GO/PARTIAL 诚实结论。复用 B068/B070 宽宇宙机制(已建只 gated)。不改策略逻辑只扩广度;研究态不可配资。spec docs/specs/B075-...-spec.md。
