...
## 当前状态
- **当前：B073 building** = 测试自动化 Phase 2.1（VCR 录放 + AI Safety Eval 网关韧性）。3 features 2g+1c:F001 VCR 录放外部 HTTP(Tiingo/SEC/LLM gateway,CI 离线确定性)/F002 网关韧性(VCR'd 确定性 red-team 硬门+live eval 区分 infra-unreachable vs advisor-unsafe,★不削弱安全门)/F003 Codex 安全相邻验收(mutation 核安全不削弱)。触发=B072 外部网关宕拖红 gate(活证)。spec docs/specs/B073-test-automation-vcr-eval-resilience-spec.md。
