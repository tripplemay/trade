...
---
## 当前状态
- **B037-OPS1-deploy-timer-sudoers-hardening：`building`**（2026-06-06 启动；横切运维修复，非 Phase 3 产品序列；来源 B037 signoff §Soft-watch S1）。**根治目标**：让新增 `workbench-*.timer` 在 `deploy.sh` 部署时自动 install+enable，根除 B035/B036/B037 三批重复手装摩擦（已沉淀 evaluator.md §24）。**根因**：`deploy.sh` L265-311 早有三段 timer install/enable 逻辑，但 VM `/etc/sudoers.d/deploy-workbench` 只授权 5 条 restart/status/daemon-reload，缺 install+enable → sudo 失败只 warn。**决策（用户批 ★）**：sudoers=★**通配符 `workbench-*.{service,timer}`**（范围锁 workbench- 前缀+systemd 目录）/ 编号=★**独立前缀 B037-OPS1**（roadmap B038-B043 不动）。2 features：(F001 g) sudoers 工件 `workbench/deploy/sudoers/deploy-workbench`(5 旧+3 新通配符)+deploy.sh 三块收敛单循环+3 守门(无残留硬编码/每 timer 有 service 兄弟/sudoers 覆盖现存)+B021 runbook 更新+security-reviewer 裁决通配符路径穿越；(F002 c) L2：admin 应用 sudoers→re-deploy 无 warn+三 timer is-enabled+active+durable 验证+关闭 S1。**Ops 前置**：sudoers 应用=一次性 admin VM 动作（deploy 无法自授权，角色边界例外须用户授权，等同 B021 #3）。spec `docs/specs/B037-OPS1-deploy-timer-sudoers-hardening-spec.md`。
