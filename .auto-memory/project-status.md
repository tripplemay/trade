...
- **B085 ✅ done（2026-07-05, round1 一轮闭环）** cn_attack 残差动量升级 first-look(评审 P2, B081驱动) = 全 2 features PASS。**★评审路线图 P0–P2 全部收官**(P0/P1/P2 first-look 均已闭环)。独立验收(代 Codex,与实现隔离): 残差/IC 从零 pandas/numpy **异路径重实现**(不import我方)51月度点逐位吻合(残差IC 0.0108/t0.45弱 vs 裸-0.0009~零; delta+0.0118/配对t1.98 borderline); β 口径 PIT 无前视(手工核对000001.SZ/2020-11-23 β=0.812397 numpy=pandas + 扰动未来β不变双证); 禁扫参 grep 零匹配(3先验常量252/126/21); trial 生产落地实测(prod alembic=0040 + B085行 bf-72732307519dec56/INCONCLUSIVE + 确定性id逐字幂等)+CI全绿+HEAD≡prod(ad8e99d)+零回归 cn_attack 产品码0行。**★rescope 引擎A/B→前置筛=合规缩窄**(完整引擎A/B需触**冻结flagship**硬边界; first-look纪律+尊边界+五处披露+保守裁定; AskUserQuestion已问用户away→按推荐option1)。**INCONCLUSIVE 弱但真实方向支持=与数字一致且保守的合法裁定**。软观察3非阻断(S1 t-stat用ddof=0/样本std复算t=1.96仍<2.0同borderline; S2 月末采样用日历月末丢~40%点但对称公平; S3 绝对IC带宇宙偏差已标注)→并入严验后续批。signoff docs/test-reports/B085-cn-attack-signal-upgrade-signoff-2026-07-05.md。完整引擎A/B降级backlog条件follow-up(待用户对触冻结flagship决策)。
