...
---
## 当前状态
- **B037-OPS1-deploy-timer-sudoers-hardening：✅ `done`**（2026-06-06 Codex F002 签收通过）。横切运维修复已闭环，根治 B035/B036/B037 三批重复手装 timer 摩擦（evaluator.md §24）。**最终生产证据（signoff：`docs/test-reports/B037-OPS1-deploy-timer-sudoers-hardening-signoff-2026-06-06.md`）**：L1 targeted safety guards `32 passed` + ruff clean + artifact secret grep `0 hits`；manual dispatch `Workbench Deploy` run `27050937093` 通过，deploy log 明示 `workbench-{advisor,market-context,prices}.timer` 三条 `install + enable` 和三条 `✓ enabled`，且**无** `Could not install/enable` warning；prod `/api/health.version=5393343134f8e639ca97ee78cfe26e5fc6696f02` 与 `main` HEAD 同 SHA；authenticated `/api/debug/recent-errors={"count":0,"records":[]}`；SSH `systemctl is-enabled/status` 证实三 timer 全 `enabled + active(waiting)`。**Durable 性成立**：`deploy.sh` 单循环 over `workbench-*.timer` + sudoers wildcard + root-owned wrapper 已覆盖未来合法命名的新 timer；Soft-watch S1 已 resolved。
