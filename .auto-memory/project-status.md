...
---
## 当前状态
- **B032-ai-safety-eval：`building`**；F001+F002 pending（generator）+ F003 pending（codex）。Spec：`docs/specs/B032-ai-safety-eval-spec.md`。
...
## 已完成签收 + MVP 完工
- B001-B031 全部签收。MVP substantively 完成 (PRD §10/§11/§12) — 完工声明：`docs/prd/mvp-completion-declaration-2026-05-20.md`。
- 最近：B031 LLM Gateway signoff 2026-05-27（1 fix-round；OpenAI-compatible API 真实接入 aigc.guangai.ai）；🎯 B030 Real Data Cutover signoff 2026-05-27（Phase 1 终点 / 里程碑 A Layer 0→1 达成）；B029 signoff 2026-05-26；B028 signoff 2026-05-26。

## 生产状态
- `https://trade.guangai.ai` live；production `/api/health.version` = `f31c3027b5a42f4585783d69eb2f84ba290352c5`；AIGC_GATEWAY_API_KEY + TIINGO_API_KEY + SEC_EDGAR_CONTACT_EMAIL 已 VM env；llm_budget_log + tiingo_budget_log 表已存在；B026 banner decommissioned + 4 protected pages BANNER_ABSENT。
...
- GitHub Secret `TIINGO_API_KEY`（B027）+ `SEC_EDGAR_CONTACT_EMAIL`（B029）+ `AIGC_GATEWAY_API_KEY`（B031）已配；B032 本批次不引入新 secret。
