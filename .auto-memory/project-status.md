...
---
## 当前状态
- **B023-workbench-phase2-manual-execution：`reverifying`**（Generator fix-round-1 已 push）；F001-F007 完成；F008 fix-round-1：scripts/test/codex-setup.sh 加 fail-fast import probe（`python -c 'import workbench_api.app'`），未装运行时依赖直接 exit 1 + 打印精确 `pip install -e workbench/backend[dev]` 补救行。本地 sanity-check：uninstall python-multipart → 探针失败带 FastAPI stacktrace；reinstall → 探针静默通过 + backend pytest 202 passed 2 skipped。共 8 features 完成 7，fix_rounds=1，等 Codex 复验 F008 L1+L2。
