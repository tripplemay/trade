...
---
## 当前状态
- **B022-workbench-phase1：`building`**；F001-F006 完成（shadcn TW3 / 7 endpoint schemas + 16 stub routes / TopBar+SideNav+ThemeProvider shell + 6 stub pages + Playwright authed / 5 chart wrappers + /dev/charts / AG Grid DataTable + 5 column helpers / Home Dashboard vertical slice：后端 services/dashboard + reports_scanner + 5 pytest，前端 4 cards + reports list + action items + empty states + 4 vitest + Playwright 4-cards 断言）；backend 109 pytest / frontend 49 vitest + 12 Playwright / 全套 lint/tsc/build 全绿。Generator 接 F007（Strategies 页 vertical slice — sleeve 列表 + per-strategy detail + provenance）。共 14 features 完成 6。预估 5-6 周。
