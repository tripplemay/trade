...
## 当前状态
- **当前：B076 building** = cn_attack size-tilt 选股(让宽池真改选股,回测验证,条件上生产)。用户选 A(B075 后宽池没改选股→改选股层)。**★verdict-gated(B069 先例)**:size-tilt 策略改动须回测验证,GO 才上生产,NO-GO 合法不改生产。size_tilt_weight 默认 0 零回归。**★回测必须用 B070 去偏 PIT 宇宙**(中小盘幸存者偏差最重),不得用 B075 当前 1490。机制=composite 加 percent-rank size 因子(US 零回归)。3 features 2g+1c:F001 机制+对照回测(sweep×两变体+中小盘广度+verdict)/F002 条件接生产/F003 Codex 独立裁定 GO/NO-GO。**很可能 NO-GO**(放开蓝筹偏向中小盘多半更差/更暴雷,诚实结论优先)。cn_attack-only 不改不可配资定性。spec docs/specs/B076-...-spec.md。
