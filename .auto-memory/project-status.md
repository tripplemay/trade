...
- **B085 🔍 verifying（2026-07-05, F001 done rescoped）** cn_attack 残差动量升级(评审 P2, B081驱动)。**F001 前置筛 done**: 残差 vs 裸动量 rank-IC(残差IC 0.0108/t=0.45弱 vs 裸-0.0009~零; delta+0.0118/配对t=1.98 borderline)→**裁定 INCONCLUSIVE 弱但真实方向支持**。★**rescope**: 完整引擎A/B需触**冻结cn_attack flagship**(硬边界), AskUserQuestion已问用户away→按推荐 option1(弱信号不为边际触冻结)收窄F001为前置筛; 引擎A/B降级backlog条件follow-up。残差计算+IC筛+trial(migration 0040)全CI绿+单测锁(隔离特质/无前视)+零回归纯研究脚本。**F002 codex 验证前置筛**(非引擎A/B, ★borderline t=1.98诚实性为重)。
