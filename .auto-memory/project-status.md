...
---
## 当前状态
- **B022-workbench-phase1：`building`**；F001-F009 完成（shadcn TW3 / 7 endpoint schemas / shell+6 stub pages+Playwright authed / 5 chart wrappers / AG Grid DataTable / Home Dashboard / Strategies / Backtest viewer (ResizablePanel + 安全 regression) / Reports (services/reports markdown 表+cross-link 抽取 + react-markdown+remark-gfm 前端渲 + heavy-table≥10 行 swap AG Grid + /docs/[...path] generic viewer)）；backend 120 pytest / frontend 59 vitest + 14 Playwright / 全套 lint/tsc/build 全绿。Generator 接 F010（Recommendations vertical slice + export markdown ticket）。共 14 features 完成 9。预估 5-6 周。
