...
---
## 当前状态
- **B035-market-context：`building`**（2026-06-04 启动；Phase 2 / Stream 2.C 第十个 batch）。Home 页市场宏观 context：FRED（10y/VIX/CPI）+ Alpha Vantage（SPY/QQQ/DXY），每日更新，复用 B027/B029 snapshot foundation。spec `docs/specs/B035-market-context-spec.md`。**F001 ✅ done**（market_context 表 + alembic 0007 + FRED+Alpha Vantage 双 adapter + 2 secret 四处接线 + fixtures；AV envelope demo-key live-validated / FRED 端点路径 no-key 400 验证为真；DXY 取 UUP 等效；backend pytest 680 / ruff/mypy 0 / alembic 0007↔0006）。**当前 sprint=F002**：scheduler 每日只读拉取（market CLI + systemd timer 非 APScheduler + deploy 接线）+ 边界 (r) AST 守门。后续 (F003) `GET /market-context` + Home 卡片；(F004) Codex L1+L2 + signoff。
