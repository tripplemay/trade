...
## 当前状态
- **B109 Tushare PIT 数据层 v0 🔍 verifying（2026-07-20, 2g+1c）** spec `docs/specs/B109-tushare-pit-foundation-spec.md`。**F001 ✅ / F002 ✅ done**，F003 交 Codex 对拍验收。
- **★★★B109 F002 计划外重大发现 — Tushare 单次调用静默截断**（`docs/audits/B109-F002-tushare-silent-truncation-2026-07-20.md`）：`income_vip` 恰 9000 行 / `namechange` 恰 10000 行，**不报错不抛异常不置标志位**，唯一线索是行数恰为整数。★**截断非均匀**：2022FY 漏掉的 1093 行里 `update_flag=0` 占 **18.7%**、`flag=1` 仅 5.2%——被砍掉的恰恰**富集 vintage 记录**。**→ F001 报告的核心数字全部作废**（分页重测 2021FY 修订率 0.525%→**1.325%**、`flag=0` 保留率 69.4%→**93.1%**；未触顶期次数字完全不变，反证偏差源）。**F001 的设计裁定（轻量两字段 resolver）未被推翻**（取决于可重建性而非修订率），但「修订稀有」印象须上调（重测 FY 期次 **1.0%–1.7%**，非原报告 0.47%–0.88%）。已修 `scripts/research/ashare_pit/fetch.py`（分页 + 触顶守卫 + FetchReport 留痕）。**待用户裁定：F001 是否分页重跑 16 期 / 是否扩测 2013-2020。**
