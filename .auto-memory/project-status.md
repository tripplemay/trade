...
---
## 当前状态
- **B034-news-ticker-embedding：`fixing`**（2026-06-04；Phase 2 / Stream 2.B 第九个 batch）。F001/F002/F003 generator ✅ done；F004 首轮 L1 全 PASS（backend pytest 643 passed+2 skipped / B034+B033 safety guards 12 passed / ruff 0 / mypy 0 / alembic 0006↔0005 / frontend lint+typecheck / vitest 176 / Playwright 39）。**L2 blocker：production authenticated `GET /api/recommendations/news?sleeve=satellite_us_quality` 返回 500**。根因已定位到运行时读取未随 deploy artifact 下发的 fixture 路径 `/srv/workbench/releases/data/fixtures/us_quality_momentum/universe.csv`，traceback 指向 `workbench_api/news/ticker_match.py::_load_universe_names()`。blocker 报告：`docs/test-reports/B034-news-ticker-embedding-blocker-2026-06-04.md`。修复方向：移除 production runtime 对该未部署 fixture 路径的依赖，或把所需 universe 数据换到 deploy artifact 内稳定可用的 runtime-safe 来源；修后重新部署交回 Codex 复验。
...
## 生产状态
- `https://trade.guangai.ai` live；production `/api/health.version` = `d1c2b30a1a09dbbfd5991e897e881703fd8fc092`（当前 `main` HEAD = `133d60950379f2c835234e6cff50483accbd0cce`，仅 `.auto-memory/project-status.md` / `framework/proposed-learnings.md` / `progress.json` 元数据差异，产品代码等价）；authenticated `/api/protected-test` OK；`WORKBENCH_DB_URL=sqlite:///var/lib/workbench/db/workbench.db`；`alembic_version=0006_b034_news_embedding`，`news` + `news_embedding` 表已存在；`/var/lib/workbench/data/snapshots/news` 已创建且为空；无 news scheduler.py / cron / systemd unit；**当前 production gap：`/api/recommendations/news` 500，因缺 `/srv/workbench/releases/data/fixtures/us_quality_momentum/universe.csv`**。
