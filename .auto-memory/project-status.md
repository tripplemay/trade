...
---
## 当前状态
- **B022-workbench-phase1：`building`**；F001-F010 完成（shadcn TW3 / 7 endpoint schemas / shell+6 stub pages / 5 chart wrappers / AG Grid DataTable / Home Dashboard / Strategies / Backtest ResizablePanel / Reports markdown rendering / Recommendations (target positions + gate panel + wash-sale empty-state + Export Markdown Ticket 写 docs/runs/<date>/order-ticket.md **含 disclaimer literal `research-only; this is a manual review checklist, not a trading instruction` — F010 safety bedrock**)）；backend 121 pytest / frontend 62 vitest + 14 Playwright / 全套 lint/tsc/build 全绿。Generator 接 F011（Snapshots + SSE refresh）。共 14 features 完成 10。预估 5-6 周。
