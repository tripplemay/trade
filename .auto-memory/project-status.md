...
## 当前状态
- **B109 Tushare PIT 数据层 v0 🔧 fixing（2026-07-20, 2g+1c, fix_round 1）** spec `docs/specs/B109-tushare-pit-foundation-spec.md`。**F001 ✅ / F002 ✅ / F003 ✅ 验收完成**。signoff `docs/test-reports/B109-tushare-pit-foundation-signoff-2026-07-20.md`；产物+9 个可重跑脚本 `docs/test-reports/B109-holdout/`。
...
- **★B109 F003 遗留 2 条 MEDIUM（待 fix）+ 1 条门异议（待用户裁定）**：**D1** `resolver.py:8,13` 仍把本批次亲手作废的 0.47%-0.88% / 98.8% 当作 as-of 设计依据陈述（`pipeline.py:259` 已明写作废，resolver 被漏掉）；**D2** acceptance 明写「替换 B076」未完成——`b076_fetch_pit_marketcap.py` 全程未触碰，违禁令 #6 的 `circ_mv_from_bar` **原样存活且被 6 个绿测背书、无弃用标记**（「替换」被做成了「新增一个更好的」）。**★I1 异议**：G-A 阈值不可证实——n=22 只支持一致率 **≥87.27%**、n=100 支持 **≥97.05%**，要断言 ≥99% 需 **n≥299 可裁定**（约 1617 份 PDF），与 spec §6 冲突 → 建议重述门为「≥N 份**可裁定**样本 0 不一致 + 100% 人工裁定」。
