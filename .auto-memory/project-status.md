...
---
## 当前状态
- **B037-home-restructure：`reverifying`**（2026-06-06 Generator fix-round 1 完成，待 Codex F004 复验）。**L2 blocker 已解**：prod `workbench-prices.{service,timer}` 已由 admin(tripplezhou)一次性 install+enable（镜像 deploy.sh；根因=deploy 用户 sudoers 无 install/enable 权，deploy.sh 只 warn，与 B035/B036 同款手装步骤漏跑，**无代码改动**）。证据：timer enabled+active next 00:30 UTC；三只读 timer(market-context/advisor/prices)全 enabled；手动 one-shot `Result=success`；journal=no-holdings(symbols=0)→`price_snapshot` 0 行属空账户合规路径(`day_pnl=null`，非 TIINGO 缺)；alembic 0009；prod `/api/health` 200 db ok。详见 `docs/test-reports/B037-home-restructure-blocker-2026-06-05.md` §Resolution + `progress.generator_fix_round_1`。**L1 早已全 PASS**：backend pytest 757 / ruff0 / mypy0；frontend lint0 / typecheck0 / vitest 198；Playwright 44。Codex 复验剩余 L2：authed `/api/home` 200 + 浏览器 Home UI/mockup/no-execution/双语 + 旧 dashboard absent + HEAD≡main。**Durable 候选**：扩 deploy sudoers 自动装 timer，消除 B035/B036/B037 反复手装（待 Planner，需一次性 admin apply）。
