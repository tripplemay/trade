...
---
## 当前状态
- **B030-real-data-cutover：`building`**；F001+F002+F003 pending（generator）+ F004 pending（codex）。Spec：`docs/specs/B030-real-data-cutover-spec.md`。
...
- 决策矩阵（2026-05-27 用户已批）：6 sector-structural ticker (BAC/JPM/V/LIN/NEE/PLD) 处置 = F001 同期 per-sector aliases 补 backfill 解决 B029 S1 / B026 banner 关闭 = B030 done by acceptance .env.production NEXT_PUBLIC_SYNTHETIC_DATA_BANNER=false (v0.9.30 §12.9 4 处接线) / 4 features 拆分 / 对比报告 = Master 4 sleeve + B025 us_quality 各独立 5 份 + 1 overview。
- 新增永久产品边界 (k)：**Layer 状态不可逆向滑落** — B030 done 后若真数据严重 unreliable 必须新批次 spec 决议，不 silent rollback。
- 本批次属 implementation-path-2026-05.md §4 **Phase 1 第五个 batch（Stream 1.D 终点）**。受益 v0.9.27 §12.7.1 paths-trigger + v0.9.30 §12.9 secret 三处接线。
- 后续路径：B030 done → **🎯 里程碑 A Layer 0→1** → Phase 2 启动（B031+ LLM advisory / B033+ News ingest / Phase 3 Home UI 部分并行）。
...
- B029 S1 (685 < 1000) 在 B030 F001 通过 per-sector aliases 解决；B029 S2 backend pytest SOCKS proxy 敏感属 evaluator 环境特定。
