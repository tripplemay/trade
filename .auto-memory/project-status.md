...
---
## 当前状态
- **B022-workbench-phase1：`building`**；F001-F008 完成（shadcn TW3 / 7 endpoint schemas / shell + 6 stub pages + Playwright authed / 5 chart wrappers + /dev/charts / AG Grid DataTable / Home Dashboard / Strategies (4 sleeves+docs sanitiser) / Backtest viewer (synthetic backtest engine deterministic seed <100ms + shadcn ResizablePanel horizontal split + 安全 regression no-resizable-outside-backtest + Trades CSV export)）；backend 117 pytest / frontend 57 vitest + 14 Playwright / 全套 lint/tsc/build 全绿。Generator 接 F009（Reports + markdown rendering，复用 F007 /api/docs/{path}）。共 14 features 完成 8。预估 5-6 周。
