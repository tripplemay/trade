...
---
## 当前状态
- **B040-reports-robinhood：`building`**（2026-06-06 启动；Phase 3 / Stream 5.A，**真实新批次非复用 gap**）。把回测指标从 quant-jargon 表格重构为 Robinhood-style：大数字+颜色编码+双语 tooltip（术语英文 Sharpe/Sortino/Calmar/MDD/CAGR + 中文 tooltip 解释）。**决策（★用户批）：两个面都做**——①`/backtest`（结构化 BacktestMetrics 现成，前端为主）+②`/reports/[slug]`（后端从异构 Markdown 指标表解析 graceful null + 前端大数字卡，**不破 body_markdown**）。planner 定：tooltip 确定性手写(AI 文案是 B043)/Calmar 派生 cagr/|mdd|/reports 解析=header-signature+同义词+graceful null(防脆弱 v0.9.21 对真实语料验证)/不新增 per-sleeve 路由。**定位 §1.1 硬边界：大数字=历史回测统计不出预期收益数字；tooltip 非 AI**。复用 radix Tooltip+/backtest MetricsCard 模板+i18n+MarkdownRenderer。4 features：(F001 g ✅done)+(F002 g)前端共享原语 MetricsDisplay+colorForMetric+双语 tooltip 接入/backtest；(F003 g)/reports/[slug]接入+body_markdown 完整性+Playwright 两面；(F004 c)L1+L2 两面手验+signoff。spec `docs/specs/B040-reports-robinhood-spec.md`。**F001✅done**（commit 9103e95，Backend+Frontend CI 绿）：ReportMetrics(7 字段 float|None)+_parse_metrics(header-signature 宽表+同义词 annualized_return→cagr 等+Calmar 派生 cagr/|mdd|+graceful null)+body_markdown 字节不变+api.ts regen+14 pytest（backend 808）。**F002/F003 前端待做（current_sprint F002）**。
