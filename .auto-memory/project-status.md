...
---
## 当前状态
- **B038-home-market-news：`verifying`**（2026-06-06 Generator F001+F002 完成，CI 全绿，待 Codex F003 L2）。Phase 3 / Stream 4.B = 把今日市场新闻搬上 Home 第三段(personas §2 mockup gap；指标卡 B037 已复用 MarketContextCard 完成)。**F001 后端**：`GET /api/news/latest`(auth 200/anon 401，全局 newest-first metadata-only，LatestNewsItem 守 B034 非生成式) + `NewsRepository.list_latest_global` + `build_latest_news`(复用确定性 tag_topics) + `workbench-news.{service,timer}`(★边界 q→r，02:00 UTC) + 守门(scope 扩 NEWS_PKG/b034 加 LatestNewsItem/news_no_scheduler 仍绿) + §12.10 + 16 单测。**F002 前端**：HomeNewsPanel(self-fetch，compact 列表 标题外链/source/日期/topic chips，三态，top8) 接入第三段 MarketContextCard 下方 + 双语 home.news + no-execution 守门扩 + vitest 7 + b037-home Daily Journey 加 home-news-card。Gates：backend 792(772+20)/ruff0/mypy0；frontend lint0/typecheck/vitest209/Playwright 绿。**F003 Codex L2**：api 200 + **B037-OPS1 durable 首验(news timer 自动 enabled 无 warn——VM 已装 wrapper+sudoers，auto-deploy 应已接线)** + 手动 trigger 验真(有行/或 SEC rate-limit 空，记录) + Home 新闻段浏览器手验+双语 + signoff。**生产 news 表此前空(边界 q)，timer 首跑(02:00 UTC)后才有数据。** spec `docs/specs/B038-home-market-news-spec.md`。**done 阶段须同步 §永久硬边界 (q)→(r) 文本 + 评估边界收编/B037-OPS1 durable 首验是否记 framework。**
