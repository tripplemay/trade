...
---
## 当前状态
- **B022-workbench-phase1：`building`**；F001-F007 完成（shadcn TW3 / 7 endpoint schemas+stub routes / TopBar+SideNav+ThemeProvider shell + 6 stub pages + Playwright authed / 5 chart wrappers + /dev/charts / AG Grid DataTable + 5 column helpers / Home Dashboard vertical slice / Strategies vertical slice：后端 services/strategies 4 sleeves + services/docs /api/docs/{path} 含 4 项 path traversal sanitization + 13 pytest，前端 DataTable 列表 + detail 面板 config/equity/drawdown/heatmap + 3 docs link buttons + ?selected deep-link + 4 vitest + Playwright DataTable 断言）；backend 116 pytest / frontend 53 vitest + 13 Playwright / 全套 lint/tsc/build 全绿。Generator 接 F008（Backtest viewer + ResizablePanel split + 跑 backtest 出图）。共 14 features 完成 7。预估 5-6 周。
