...
---
## 当前状态
- **B034-news-ticker-embedding：`building`**（2026-06-01；Phase 2 / Stream 2.B 第九个 batch）。在 B033 news 基础设施上加语义关联层，为 B036 AI advisor 提供 news↔sleeve 关联 + 可引用 news URL/SHA。spec `docs/specs/B034-news-ticker-embedding-spec.md`。**F001 ✅ done**（`news_embedding` 表 + alembic 0006 + NewsEmbedder bge-m3 + 离线 fixture 向量 + S1 env 闭合 + 非生成式守门；bge-m3 live-validate dim=1024）。**F002 ✅ done**（ticker_match 全词匹配 + 确定性 topic taxonomy + sleeve_tickers + NewsAssociationService hard ∩ + 纯 Python cosine 软排序；backend pytest 630 全绿）。**当前 sprint=F003**：`GET /recommendations/news`（auth-gated same-origin 纯结构化无 AI 文本）+ 前端 NewsPanel（topic chip + score 排序 + 筛选）+ vitest/Playwright。后续 (F004) Codex L1+L2 + signoff。
