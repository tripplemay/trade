...
---
## 当前状态
- **B022-workbench-phase1：`reverifying`**（fix-round 4）。Round-3 deploy 后 Codex 通过新的 /api/debug/recent-errors 拿到了真根因 — `OperationalError: no such table: backlog_entry / snapshot_meta`。即 prod SQLite 缺表。本轮 1 个 root-cause 修复 + 1 个永久 guard：**workbench/deploy/scripts/deploy.sh 在跑 `alembic upgrade head` 前没 source /etc/workbench/workbench.env**。SSH 会话不像 systemd EnvironmentFile= 那样自动 load env file，所以 alembic 看不到 WORKBENCH_DB_URL → Settings 走 DEFAULT_DEV_DB_URL='sqlite:///./workbench-dev.db' → 迁移落到 ${RELEASE_DIR}/backend/scratch DB 上下个 deploy 就 GC，真实 /var/lib/workbench/db/workbench.db 从未 migrate。B021 路由全是 SELECT 1 + JWT cookie 不读表所以没暴露；B022 是第一个 request-time 用表的批次。修复加 `set -a; . ${ENV_FILE}; set +a` 让 alembic 子进程看到 WORKBENCH_DB_URL；额外加 post-alembic schema verifier 用 sqlalchemy.inspect 校验 account+backlog_entry+snapshot_meta 真在 WORKBENCH_DB_URL 指的 DB 里，缺则 exit 1。Regression: tests/safety/test_deploy_alembic_env_load.py 4 测试 grep deploy.sh（exists / source 句式 / 出现序在 alembic 前 / verifier 提全 3 表）。本地 smoke 通过：WORKBENCH_DB_URL=sqlite:////tmp/x.db alembic upgrade head → tables [account, alembic_version, backlog_entry, snapshot_meta]。本地全绿：backend pytest 139+2 skipped(135→139, +4 deploy safety)/ruff/mypy 94 文件；本轮无 frontend 改动。等下次 deploy 跑后 Codex re-walk L2 10 项；deploy log 出现 '✓ schema check passed: [...] present in sqlite:////var/lib/workbench/db/workbench.db' 这行就是迁移落对位置的证据。
