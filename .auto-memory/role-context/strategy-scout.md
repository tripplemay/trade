...
**★2026-07-21 用户裁定:新信号搜索正式冻结。** 已测 24 次 0 次可部署(B069→B110 + 07-11/07-12 八份 first-look),
Laplace 估计 P(下一个有 edge) ≈ **3.8%**;条件期望上界(B110 最乐观格 1.1586% 毛超额 − 成本 1~2%/年)= **−0.8%~+0.16%**。
**重开条件仅限「数据类别变了」**(日内 / L2 / 真实持仓流 / 新资产类),**不含「再测一个因子」**。
Scout 本轮起**不得**因发现新因子候选而自动开批;发现只写简报 + backlog,由用户决定。
...
- 聪明钱:游资跟随(B094 反亏)、大股东增持大盘(B101 真空)、龙虎榜粗糙机构 tag(B103 显著负);
  免费聪明钱四支已穷尽;付费 Tushare `top_inst` **亦不再列为候选**——B105 已实质回答:
  即使 IC 站住,**90-107% 净收益在 A 股不可做空的短腿**,多头腿 −0.0011/月,付费买不回市场结构性做空限制。
- hk_china 真个股(B093 NO-GO 保 proxy,数据缺口除外);
- **★纯 E/P(B110 NO-GO,2026-07-21 用户裁定更正 evaluator 的 INCONCLUSIVE)**:多头腿 alpha −0.3237%、
  Q4>Q5 在 4 变体×2 基准 8/8 全成立。★**该裁定对预注册窗口 2013-2024 成立;实测若丢掉 2013 年会翻成
  INCONCLUSIVE(+2.94%/顶层最优)——窗口杠杆(10.89pp)大于数据质量杠杆(2.05~3.96pp),
  故任何重测必须先预注册窗口。**
- **★微盘 / MV1 五分位(2026-07-21 对抗评估确认否决正确)**:头条 +37pp 里 >50% 是**滞后一月即消失的
  微结构伪影**(Blume-Stambaugh 价格误差指纹);2017-2020 beta 调整后 alpha t=**0.06**;
  **15 个月真实样本外反向**——2026 H1 万得微盘股 −7.2%(回撤 −25.07%) vs 中证1000 +15.99%,
  归因明写「退市新规使壳价值溢价被持续压缩」。
- **★B106 组合层三项**(红利低波并入 Master 杠铃 / risk_parity sleeve 配权 / HRP)全部 NO-GO:
  相关性 **+0.114~+0.478(不是负)**→分散前提不成立;HRP ΔSharpe −0.0542。
- **★07-11 / 07-12 八份 first-look 全部 NO-GO 或 DATA_NO_GO**:短期反转、low-MAX、SUE-PEAD、
  多尺度价量、ETF 时序趋势严验、红利价值 H30270、**红利质量低波(指数级 931130,与股票层已实现 σ 不同命题)**、
  增持小盘、机构建仓季报、US 攻击选股全 build。

> 详见各 signoff:`docs/test-reports/B0XX-*-signoff-*.md`;裁定全景
> `docs/research/ashare-strategy-campaign-final-verdict-2026-07-07.md`;
> B110 更正 `docs/test-reports/B110-F004-adjudication-correction-2026-07-21.md`。

## §已验证为正的是「风险层」不是「收益层」(2026-07-21 沉淀)

**★每一次拿到真实正结果的测试,测的都是风险不是预测收益**:B082 MaxDD −25.7pp、
B089 急跌 −34.1%→−15.2%、B110 副产降波 +1.28pp/年。而 24 次「预测收益」测试 0 胜。
**Scout 的候选筛选应据此重排优先级:风险/组合/执行层 > 选股信号层。**
