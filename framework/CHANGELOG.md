...
## v0.9.57 — 2026-08-06（B112：golden 快照不依赖 git show / 浮点等价数值容差 / 覆盖披露逐段 / spec 冻结输入全窗口可得性 / HEAD 可复跑 / 拼接防并集污染）

**来源批次：** B112（防御闸 A/B 裁定批，两轮修复；单条提案含 6 子内容，用户确认后沉淀）。

**沉淀内容（用户确认，6 条）：**
- **golden 测试基线用提交快照/指纹，不在运行期依赖 `git show old-sha`；golden 前必须 canonicalization** → `evaluator.md §38`（B112-F001-5a：CI 浅克隆 fetch-depth:1 拉不到历史 commit；float `.14g`、set 排序、date isoformat，否则 hash seed/1-ULP 噪声造假回归；落地实例 commit `649c22f`）。
- **浮点回测产物的等价判据是数值容差，不是 canonical JSON hash** → `evaluator.md §39`（B112 signoff S1：同输入跨进程 ≤3e-13 IEEE 末位噪声；hash 不同必须展开字段级 diff，≤1e-10 且仅数值末位可接受，超阈或非数值字段变化即 blocker）。
- **覆盖披露必须逐段——名称、行数、跨度、去重优先级** → `evaluator.md §40`（B112-F001-4a：只报合并总数漏掉占半数以上的输入段，3.2M 行新名价格段漏报 + 指数「日历日」误标「交易日」）。
- **spec 冻结输入条款落笔前必须核实全窗口可得性，预设「不可得 → 数据缺口」分支** → `planner.md §spec 冻结输入条款落笔前必须核实全窗口可得性`（B112：top~1500 去偏 PIT 宇宙全窗口不可得，直到验收才暴露）。
- **「提交前可运行」升级为「HEAD 可从原始输入复跑」** → `generator.md §50`（B112-F001-2：本地未提交修补 = 验收方在 HEAD 复跑即死；交付含 runner 的产物前必须在干净 stash 状态下从原始输入冒烟）。
- **数据集拼接防方法论并集污染——同源优先 + 异源补新块** → `generator.md §51`（B112 实测：同 as_of 异源宇宙块按 `(as_of,ticker)` 并集造出 1700+ 名伪宇宙）。

归档 `framework/archive/proposed-learnings-archive-v0.9.57.md`。**活跃候选队列清空。**
