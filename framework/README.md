...
### 回测保真度：引擎修真 A/B 必须配本金扫描（来自 B081，★F005 审计更正）
- **元教训（成立）：宣称某回测有 edge 前，先跑「引擎修真 A/B」**——把手数取整（100 股/手）、停牌、退市、涨跌停各做成开关，逐一验证 edge 不是引擎理想化假象。（B081 F002 停牌/退市对*流动动量票*= no-op，"最重高估源"对本策略零影响；但对持 *ST/低流动名的策略才咬——修真项是否咬取决于策略持仓。）
- **★但「手数取整」组必须同时做本金扫描（100k / 1M / 10M），否则会把容量下限误读为策略失效**：B081 A/B（B070 去偏 PIT，pure_momentum）初看 `lot@100k` OOS **−14.7%**（旧分数股口径 +28.4%），一度被误读为「边际大半是分数股假象」。**F005 独立数字审计证伪此叙事**——负数是 **10 万本金的容量下限**（25 只等权中约 9 只一手买不起 → 被 skip → 现金拖累 + 高换手），而非分数股假象：`lot@1M` OOS **+23.5%**、`lot@10M` **+28.2%**（**保留 99% edge**）。edge 随本金放大即恢复 = 容量约束，不是策略失效。
- **规约：**（1）手数取整 A/B 必带本金扫描；（2）宣称「某修复揭示假象」前，先问**该效应是否随本金/规模消失**（消失 = 容量下限，非假象）；（3）**A/B 结论本身也要过独立数字审计**——B081 F005 正是抓住了 F004 把容量下限误读为策略失效。
- 配套 generator.md §45（慢真机回测 resumable + pickle 缓存抗 kill，跑这类多组全宇宙 sweep 的基建）。

### 外部数据源完整性不可假设（来自 B109 F002）
- **「调用成功返回」≠「拿到了全部」**：Tushare 单次调用静默截断（`income_vip` 恰好 9000 行 / `namechange` 恰好 10000 行）——不报错、不抛异常、不置标志位，唯一线索是行数恰好是整数页。
- **截断非均匀**：被砍记录富集 vintage（`update_flag=0` 占 18.7%），而那正是修订率/保留率的计算依据 → B109 F001 已交付核心数字全部作废（分页重测 2021FY 修订率 0.525%→1.325%）。
- **规约：凡分页接口，要么分页拉取，要么显式证明未触顶，不存在第三种可接受状态。**
- **撤回失效常量时留 `None` 而非留旧值**——已知偏低的数会被下游当已验证事实引用，比留空更危险。

### 退役判据：让误用不可能悄悄发生，而非让代码消失（来自 B109 F003）
- 提删除/退役建议前**必须先查消费者**：B109 一度建议删 `b076_fetch_pit_marketcap.py`，复验发现其产出 CSV 正被生产策略读取——删除会切断生产在读数据的 provenance。
- 存在合法消费者时目标态是「警示但可用」（⛔弃用横幅 + `DeprecationWarning` + 测试固化禁令），不是删除——「可用」与「该用」是两件事。
- 弃用契约要有牙齿：变异测试验证（注释掉 `warn` → 测试必须红）；`DeprecationWarning` 在**默认 filter** 下须实证可见（库代码中默认被隐藏）。
- **禁令有射程**：跨射程套用会制造假缺陷（禁令 #6 射程是 E/P 分母，不管规模因子排序量的 `circ_mv`）。

### 裁定纪律：敏感性触发≠改档、析取项≠析取式、「保守」须分对象（来自 B110 F004）
- **敏感性/口径条款「触发」≠「改变裁定」**：触发后必须机械展开歧义两端并验证是否**跨越判据边界**——B110 两端（+0.96% / −0.76%）都 ≤1.0% NO-GO 线，D1 空转不得据此改档。配套**不对称性自检**：把实测值换成刚好跨对侧阈值的假想值，同一套规则会裁什么？两侧裁法不一致 = 条款被非对称使用。
- **推翻一个析取项 ≠ 推翻析取式**：OR 结构判据中，敏感性条款只能挑战它实际影响的那个析取项（B110 第二析取项是组间比较，基准不进入，8/8 单元格一致 NO-GO）。
- **「保守」须限定对数据还是对资本**：花钱决策里 INCONCLUSIVE 是**放行档**（「搞好数据再看一次」），NO-GO 才是保守档——first-look 裁定档必须标注各档的资本行动含义（放行/收口）。

### 环境迁移当日必须同步 environment.md（来自 B111，第三次同类：B065/B067/B111）
- 生产服务器迁移（GCP→deploysvr）后 environment.md 未更新 → agent 对已退役 IP 重试 17 次 ×5 小时误判「fail2ban 封禁」。同型教训已沉淀两次仍复发——「连接失败先核对 IP」规约**依赖 environment.md 本身是对的**。
- **规约：任何环境迁移（服务器/域名/端口/账号），迁移动作不闭环到 environment.md 更新不算完成；连接失败先核对 `~/.ssh/config` 别名与 environment.md 是否一致。**
