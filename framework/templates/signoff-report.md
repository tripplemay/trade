...
## 实测证据（决策级/真数据批次必填）（v0.9.45 — B061/B062/B063 三例沉淀）

> **触发：** core acceptance 为「真实数据 / 真机行为 / 决策依据数字」时**必填**；纯框架 / 纯文档 / 纯前端批次写 "N/A"。
> **规约（evaluator.md §29）：** 逐条贴**实际观测**，**缺任一实测 = 不得 done**。任一行写「未执行 / 受阻 / 框架就绪 / ready for execution」→ 该 acceptance 项判 **CONDITIONAL，不计入 done**。「部署存在 / 代码结构论证 / 数据源存在」**不是**实测证据。

| acceptance 项 | 实测证据（真返回行 / pre-post 数字 / 决策指标）|
|---|---|
| [真返回行/值] | 例：`0700.HK → 5405 行腾讯, 收盘 463.6@2026-06-12`（非「端点存在」）|
| [pre/post 对比] | 例：`US-Master 推荐 pre=20 / post=20 positions 逐项一致`（非「未改代码推定不破」）|
| [决策依据数字] | 例：`proxy CAGR +2.77% / Sharpe 0.55 vs real −0.06% / −0.32` + **go/no-go 明确结论**（决策点批次）|

---

## 裁定核对清单（含冻结条款 / 敏感性判据批次必填）（v0.9.56 — B110 F004 沉淀）

> **触发：** 批次 spec 含预注册冻结条款（附录 D1/D2/…）或敏感性/口径条款参与裁定时**必填**；无冻结条款批次写 "N/A"。
> **规约（evaluator.md §36）：** 冻结条款必须**逐条列出编号**表明已核对——「把自己漏读的已存在规则记成框架缺失」是已发生过的自欺形态（B110 D8 逐字存在却从未被引用，反被记为框架缺口）。敏感性/口径条款触发后须机械展开歧义两端并验证是否**跨越判据边界**，未跨越 = 条款空转、不得据此改档（B110 D1 两端都 ≤1.0% NO-GO 线）。裁定档须标注资本行动含义（放行/收口）——「保守」须限定对数据还是对资本。

### 已逐条核对的冻结条款清单

| 条款编号 | 内容摘要 | 核对结论（触发 / 未触发 / 空转-未跨判据边界） |
|---|---|---|
| [D1] | [摘要] | [结论] |
...
| 项 | 结果 |
|---|---|
| 原始判据触发项 | [哪条判据实际触发，如「年化超额 ≤1.0%」/「顶层组不是最优」；OR 判据须指明是哪个析取项] |
| 敏感性覆盖项 | [敏感性/口径条款是否改变裁定；若改档：歧义两端展开值 + 是否跨判据边界 + 不对称性自检结果] |
