...
## 35. 退役判据——提删除建议前必须查消费者；弃用契约须有牙齿（v0.9.56 — B109 F003 沉淀）

**规律（evaluator 自我更正实例）：** B109 F003 首轮建议「直接删除 `b076_fetch_pit_marketcap.py` 及其测试」，复验追 provenance 发现该脚本产出的 `data/research/b076/cn_size.csv` 正被**生产策略**（`trade/strategies/cn_attack_momentum_quality/size.py` + B080 拥挤度监控）读取——删除会切断生产在读 CSV 的 provenance，反而抬高「未来有人重搓错版本」的风险。正确解 = 保留本体 + ⛔弃用横幅 + 运行时 `DeprecationWarning` + **测试固化禁令**。

**规约：**
1. **退役判据是「让误用不可能悄悄发生」，不是「让代码消失」**：提删除/退役建议前必须先查消费者（grep 产出物引用 / import 链）；存在合法消费者时目标态是「警示但可用」——「可用」与「该用」是两件事。
2. **变异测试验证弃用契约有牙齿**：注释掉 `warnings.warn` 后测试必须 FAIL（B109 实测 4/7 FAIL），证明禁令由测试守住而非注释里的君子协定。
3. **`DeprecationWarning` 须实证在默认 filter 下可见**——库代码中它默认被隐藏，不能想当然。
4. **禁令有射程，勿跨射程套用**：`size.py` 用 `circ_mv` 不违禁令 #6（#6 射程是 E/P 的**分母**；`circ_mv` 是规模因子排序量，规模因子惯用流通/自由流通市值）——跨射程套用会制造假缺陷。

**来源：** B109 F003（evaluator 复验时自我更正删除建议，git 核实消费者）。配套 README §经验教训「退役判据」。

## 36. signoff 必列已核对冻结条款清单；一致率指标必报分母筛选逻辑；禁恒真断言（v0.9.56 — B110 F004 沉淀）

**(a) 已逐条核对的冻结条款清单（防「漏读记成缺失」）：** B110 冻结附录 **D8 逐字写着**「顶层组不是最优 → NO-GO 照字面执行（预注册锁死，不得改）」并提前否决「统计不显著」反驳，而 signoff 全文 D8 仅在 `D1-D8` 区间写法中出现一次、**从未作为规则被引用**，soft-watch 反把「未定义优先级」记为 medium 框架缺口——**把自己漏读的已存在规则记成框架缺失，按原样沉淀会去补一条已存在的规则，掩盖真实教训**。**规约：signoff 裁定段必须逐条列出已核对的冻结条款编号清单**（见 templates/signoff-report.md），漏读因此在报告结构上可见。

**(b) 一致率/通过率指标必须同时报告分母筛选逻辑（防抽样池自欺）：** B110 F004 声称「四分量 240/240 R2 对拍 MATCH，BREAK=0」，但抽样池是 `ep` 非空的行，而 `ep` 非空 ⟺ TTM 可用 ⟺ R2 已全 MATCH（BREAK 会 fail-closed 掉 `ep`）——**BREAK 样本按构造不可能被抽到**。实测：可达池随机 400 条 BREAK=0；被排除池随机 400 条 BREAK=78（与全量 7,949/35,152 = 22.6% 自洽）。★加重情节：本批冻结附录 §4 逐字写明这是「与 B109 审计器同型」的自欺陷阱并把 R2 列为「替代的真校验」——**明令警惕的形态被原样搬到替代校验上**。**规约：任何「一致率/通过率」指标必须同时报告『分母是怎么筛出来的』，并检查筛选条件是否与被检项逻辑相关。**

**(c) 禁恒真断言：** F004 漏斗闭合断言 `(a−b)+(b−c)+(c−d)+d ≡ a` 是望远镜恒等式，捏造数字也返回 True、`raise` 不可达——**恒真断言与同义反复抽样是同一类错误的两种表现**，验收时视为无证据。

**来源：** B110 F004（用户裁 NO-GO 推翻 evaluator 的 INCONCLUSIVE；可达池 400 条 BREAK=0 vs 被排除池 400 条 BREAK=78）。配套 §29（实测证据硬段）+ generator.md §47（几何/算术并排）。

## 37. 生产状态修复 signoff 必须三件套证据——故障快照 + 恢复账本 + 纯函数重放（v0.9.56 — B111 沉淀）

**规约：** 仅凭一次 CLI 输出不可签收生产状态修复。B111 标准做法 = **三件套同时具备**：①**故障时刻快照** + ②**恢复后账本** + ③**纯函数重放**（读取故障时刻快照，用生产 SHA 的引擎重放，与恢复账本逐位吻合）。CLI 输出只证明「现在看起来对」，重放才证明「修复逻辑对」。

**来源：** B111（生产状态修复验收）。配套 §29（实测证据硬段）+ §25.1（正面证据）。

## 38. golden 测试基线用提交快照/指纹，不在运行期依赖 git show old-sha（v0.9.57 — B112 沉淀）

**坑：** golden 测试在运行期 `git show <old-sha>` 取历史基线，CI 浅克隆（`fetch-depth: 1`）拉不到历史 commit → 假红。★次生坑：golden 落盘前不做 canonicalization，hash seed / 1-ULP 噪声会制造假回归。

**规约：** 固定历史基线必须**提交 golden 快照/指纹进仓库**，测试只读工作树文件，不得在运行期依赖 git 历史；golden 落盘前必须 canonicalization——float 格式化 `.14g`、set 排序、date `isoformat`。

**来源：** B112-F001-5a（落地实例 commit `649c22f`）。配套 §39（浮点等价判据是数值容差）。

## 39. 浮点回测产物的等价判据是数值容差，不是 canonical JSON hash（v0.9.57 — B112 沉淀）

**坑：** 同输入跨进程存在 ≤3e-13 的 IEEE 末位噪声，用 canonical JSON hash 对拍浮点回测产物必假 BREAK。

**规约：** hash 不同**必须展开字段级 diff**：差异 ≤1e-10 且仅落在数值末位可接受放行；超阈或任何非数值字段变化即 blocker。

**来源：** B112 signoff S1。配套 §38（golden canonicalization 纪律）。

## 40. 覆盖披露必须逐段——名称、行数、跨度、去重优先级；只报合并总数无效（v0.9.57 — B112 沉淀）

**坑：** 多段数据合并只报合并后总数，会漏掉占半数以上的输入段——B112-F001-4a 的 3.2M 行新名价格段漏报全程未被发现；指数「日历日」被误标「交易日」同例。

**规约：** 覆盖/合并披露必须**逐段**列出：段名称、行数、时间跨度、去重优先级；合并总数只能作为逐段表之外的汇总行，不得单独充当披露。

**来源：** B112-F001-4a（H6 覆盖披露漏报实例）。
