...
## 33. feasibility-first 探针复用真生产 loader——既测可行性又验代码路径（v0.9.51 — B075 F001 沉淀）

**规律：** §23 类 feasibility-first 探针（"先验该数据源/规模可行吗"）应**直接调真生产 loader**（B075 探针用 `discover_ashare_superset` / `CnHkPricesLoader` / `CnMarketCapLoader` / `CnFundamentalsLoader`,非裸 lib、非合成）——这样**一次 VM 实跑同时验证两件事**：(a) 数据源可行性（sina 发现 1501 / 日刷 100% 成功）+ (b) **新 ungate/生产代码路径真能在 VM 跑通**。对比 b070_feasibility_probe 用裸 baostock（只测数据源、不测代码路径）:复用真 loader 的探针多覆盖代码路径回归,GO 结论更可信。**规约：** 写可行性探针时优先 import 真生产 loader/构建函数,而非重写旁路调用;探针通过 = 可行性 + 代码路径双绿。

**来源：** B075 F001（VM 探针复用真 loader,GO@N=1501 同时证 ungate 路径 VM 可跑;commit `7496d80`）。配套 §23（新端点实跑）。

## 34. 宽集逐只刷 job 的 partial-failure exit-code 容忍——尾部失败不炸整轮（v0.9.51 — B075 F002 沉淀）

**坑：** 大宇宙（~1500）逐只刷必有**尾部失败**（退市/停牌/个股端点抽风）。旧 `main()` 的 `errors>0 → exit 1` 会让**日 timer 天天假阳报红**（明明 99% 成功）。
**修法（B075）：** 宽块错误**单列计数**（`cn_universe_price_errors` / `cn_fundamental_errors`,同时并入 `errors` 总数 = 维持单一 error 契约）+ `resolve_exit_decision` 分层：**core 块（US / CN_HK 主集）错误严格**（任何失败→红）;**宽块按 rate floor 容忍**（失败率 ≤20% 算正常尾部,绿）;**只有真停摆**（host down → 大批失败超阈）才红。
**规约：** 任何"宽集逐只刷"job（CN 宽宇宙 / 未来任何 universe-scale fetch）的 exit-code 必须**区分 core-严格 vs 宽块-rate-floor**,否则可恢复的尾部失败污染 timer 健康信号。是 §28「partial-failure 优雅不炸整轮」的 exit-code 层落地。

**来源：** B075 F002（宽块 error 单列 + `resolve_exit_decision` rate floor;commit `ae9dc42`）。配套 §28 + planner.md spec「partial-failure 优雅」。

## 35. 去偏策略-改动研究两件套——退市名市值补数据（baostock turn 反推）+ survivor/去偏双 cut（v0.9.52 — B076 沉淀）

**(a) 退市名市值/估值因子补数据法（refines B070 去偏）：** 去偏 PIT 回测（含退市名）若要加**市值/估值类因子**（size-tilt / value），`stock_value_em`(eastmoney) 只覆盖**当前上市**名 → 退市名缺市值 → 因子静默丢名 = **重引入幸存者偏差**（本想去偏反而又偏）。**修法：用 baostock k-data 的 `turn`（换手率%）反推流通市值** `circ_mv = close_raw × volume × 100 / turn`（`adjustflag=3` 未复权;`turn = volume / 流通股本 × 100`）。baostock 对所有有 k-data 的名（含退市）都给 turn → 覆盖率 100%（B076: 1310/1310,names_empty=0）。**通用于任何需退市名历史市值/估值的去偏研究。** 补完须断言覆盖率（缺名因缺因子被丢 = 偏差泄漏）。

**(b) survivor / 去偏双 cut 验收范式：** 同一策略改动可能 **survivor 宇宙=GO、去偏宇宙=NO-GO**（B076: survivor B068 quality_momentum Sharpe 1.00→1.27 vs 去偏 B070 pure_momentum 0.56→0.42——退市小盘输家在 survivor 宇宙缺席,美化中小盘 tilt）。这是「回测必须去偏」最强铁证。**范式：primary = 去偏宇宙做 gating 裁定;secondary = survivor 宇宙仅方向性参考,显式标注「survivor GO 不足为凭、survivor NO-GO 更可信」。** 不得用 survivor cut 当 GO 依据。

**来源：** B076 F001（size-tilt 去偏 sweep;commit `e6cc2c4`+`047c49f`+`c4bd82c`,Codex 独立复跑 bit-identical 裁定 NO-GO）。配套 §23 + B070 去偏 + planner.md §「策略-改动 verdict 设计」。

## 36. §23 measured-not-assumed 要贯彻到每个派生字段,非仅数据本身（v0.9.53 — B077 F001 沉淀）

**坑：** 可行性/数据现实探针**即便数据是 VM 实测的**,若 data-reality 的**派生标签/裁定字段被 hardcode** 仍违反 §23（B077 F001: `coverage="full_market"` 常量 / `lag_days=0` 写死 / `can_support_backtest=True` 不门控;20-agent review 抓到 15 处）。**规约：探针每个 reality 字段（coverage / lag / depth / backtest-supportability）必须从实测派生**——lag 由真实日期算、coverage 由实测 snapshot 反映、backtest-supportability 由实测历史深度门控。review 设专门 lens 查「标签是真实测还是假设」。

**来源：** B077 F001（三源数据可行性,coverage/lag/can_backtest 曾 hardcode）。配套 §23（新端点实跑）。

## 37. first-look IC 覆盖-门控裁定档 INCONCLUSIVE_COVERAGE_LIMITED（v0.9.53 — B077 F002 沉淀）

**规律：** 去偏宇宙做信号探查时,若信号总体**大部分落在去偏宇宙外**（B077: 龙虎榜机构席位 80.8% 事件是小盘异动,B070 流动宇宙小盘天花板未覆盖）→ first-look IC 只测到子集,据此断「无信号」是误劝退。**修法：裁定加第三档**——faint 一致方向（|IC|≥0.015 + 多 horizon 同号）+ 覆盖率 <50% → `INCONCLUSIVE_COVERAGE_LIMITED`（既不 GO 也不直接劝退,决定性检验=补覆盖重跑）。配套：分组均值须查**单调性**（B077 驼峰:极端净买均值回归,正 top-bottom 非单调梯度）,勿据 top-bottom 同号即称「方向真实」。

**来源：** B077 F002（LHB 机构席位 rank-IC 0.018-0.023/覆盖 19.2%/驼峰非单调）。配套 planner.md §策略-改动 verdict 设计 + B070 去偏。

## 38. 宽集刷 job 防挂死的超时要包裹 ALL 真网络调用——bulk discovery 最易漏（v0.9.54 — B078 F001 沉淀,扩 §34）

**坑：** per-call 超时只包「逐只」fetch 不够：**bulk 发现/快照调用**（`stock_zh_a_spot_em` / `stock_zh_a_spot`）常在逐只循环 **BEFORE** 跑、且无超时——挂死则 0 数据写入、命门再冻（B078 对抗式自审抓到,spec §1 清单只列逐只漏了 bulk）。**规约：** 宽集 job 超时清单须含 **(a) 逐只 fetch (b) bulk discovery (c) benchmark/单次 index 等所有真网络调用**;原语用 **daemon-线程 + `join(timeout)`**（跨平台/可嵌套;超时则 leak 的线程是 daemon 不阻塞进程退出;`timeout=0/None` → inline 调用保证既有路径零回归）。写完 grep 核「无 timeout 的网络调用」残留。

**来源：** B078 F001（A股 data-refresh 挂死,bulk discovery 漏超时;commit 见 B078）。配套 §34（partial-failure exit-code）+ evaluator.md §systemd oneshot 卡死诊断。

## 39. paper 再平衡成本要预留 round-trip(买+卖),单边预留满仓必透支（v0.9.54 — B078 F002 沉淀）

**坑：** `investable = equity*(1-cost_rate)` 只够付**买入**腿成本;调仓在 gross=买+卖 全程收费,高换手（全换）按卖出腿透支 ≈ `held*cost_rate`（B078: B074 剥 CASH buffer 满仓后 cash -102/-103）。**修法：** `investable` 再减 `held_marked_value * cost_rate`（卖出腿上界=持仓市值）→ 任意换手 `cash ≥ 0`（可证 `new_cash ≥ r²(equity+held)`）;from-cash 建仓 `held=0` → 公式不变 → **建仓零回归**（仅高换手调仓变）。通用于任何「满仓+成本」再平衡引擎。

**来源：** B078 F002（cn_attack paper 负现金 -102/-103,B074 剥 CASH sentinel 满仓副作用;reverify cash +187.52 转正）。

## 40. 静默冻结守门——数据 as_of 业务日新鲜度 + service stuck-activating 双断言（v0.9.54 — B078 F002 沉淀）

**坑（最毒）：** 数据管道挂死**无人报错**（B078 冻 4 天 precompute 每天重吐同快照、paper 照「跟」冻结目标,无任何红信号）。**规约：** 把 **(a)「快照 as_of 业务日年龄 ≤ N」+ (b)「service 不 stuck-activating > X 时」** 做成纯函数 + acceptance 守门（**teeth:故意造陈旧→红**;业务日计数免疫周末,长假残留 WARNING 可接受;**teeth 测须 pin SHIPPED 默认阈在真实冻幅**,否则默认回归静默重开 bug）。是「验收即代码 §31」对运行时数据新鲜度的延伸。

**来源：** B078 F002（A股 冻 4 天无人知;新鲜度守门 acceptance）。配套 §31 验收即代码 + §38/§39。

## 41. 编辑 `trade/` 触发 spec 未列的额外门禁 family——子集绿 ≠ 全绿（v0.9.55 — B080 F002 + B081 F001 沉淀）

**规律：** 一个 feature 若**编辑到 `trade/` 源**（取 trade/ 对象新字段落库、或翻转 backtest 默认口径），会触发 spec「gates 同 F00x」漏列的额外门禁。两类实例：

**(a) 从 `trade/` 源对象取新字段落库（B080 F002）：** F002 把 cn_attack 复合因子分从 `CnSignalResult.factor_contributions_dict()` 提到 `CnAttackLiveTarget` 数据类——一处 `trade/` 编辑，因而触发 spec 未列的门禁：`mypy trade`（Python CI）+ 根 `ruff check .` + 根 pytest（cn_attack live / b075 / b076 acceptance）+ **改 `trade/` 后必须 `.venv/bin/python -m pip install ../..` 重装 trade 到 backend venv**（否则 backend 测试导入的是 stale 已装副本，报 `no attribute signal_scores`——同 §26.3 copy-install 坑）。

**(b) 翻转 backtest 默认口径 / 默认 config 值（B081 F001）：** 改变 backtest **默认口径**的 `trade/` edit（config 默认值翻转，如 `lot_rounding: bool = True`）**不能只跑 cn_attack 子集就 push**——full root pytest 里有其他测试消费默认 backtest（comparison / reporting / **overfitting detector**）。B081 `lot_rounding` commit `e94955f` 只本地跑 cn_attack 子集（65 测绿），push 后 Python CI 在 `test_implausible_sharpe_flagged` 红：round-lot cash-drag 把小合成账本 Sharpe 压到 implausible 阈值以下，检测器不再触发。修复 = `build_cn_attack_comparison` 透传 `base.lot_rounding`（之前 per-variant cfg 丢弃它）+ 该测 pin 旧口径。**另一坑（同批）：A/B comparison / 多变体构造器（`build_cn_attack_comparison` per-variant cfg）新增 config switch 时易漏透传 `base.<switch>` → 比较静默忽略 caller 口径；加 engine switch 时须同步透传所有多变体构造点。**

**规约：**
1. **凡编辑 `trade/` 源的 feature**，push 前把门禁清单当作「spec 的 gates 段 + `mypy trade` + 根 `ruff check .` + 根 pytest（消费 trade/ 的 acceptance 子集）+ backend venv 重装 trade」，**不信 spec「gates 同 F00x」的字面覆盖**。
2. **改 backtest 默认口径/默认 config 值**，push 前必须本地跑 **full root pytest**（或至少 grep 所有 `run_cn_attack_backtest` / `build_cn_attack_comparison` 消费点的测试文件全跑）。**子集绿 ≠ 全绿。**
3. **加 engine switch 时同步透传所有多变体构造点**（`base.<switch>` → per-variant cfg），否则多变体比较静默忽略 caller 口径 = §17 plumbed-but-ignored。
4. planner 起草 spec 时，凡涉及从 `trade/` 源对象取新字段 / 改默认口径的 feature，acceptance 的 Gates 段应显式列这些（见 planner.md）。

**来源：** B080 F002（signal_scores 落库,commit `dd9f703`+`46ba83b` 系）+ B081 F001（lot_rounding 默认翻转,commit `e94955f`）。同族 §19（本地门禁 CI-exact）+ §26.3（workbench venv trade copy-install 须重装）。

## 42. pydantic 加带默认值字段 → openapi-ts 仍标 required → 前端 fixture 须同 commit 补（v0.9.55 — B080 F004 沉淀）

**坑：** 给 pydantic response schema 加**带默认值**的字段（如 `PaperSummary.benchmark_symbol="SPY"` / `first_day_caveat=False`）时，`openapi-typescript` 生成的 TS 类型仍标为 **required**（响应恒含该字段 → 无 `?`）。因此所有用字面量构造该类型的前端 fixture（`tests/unit/**/paper.spec.tsx`、`PaperPage.spec.tsx`）会立即 `tsc` 失败。**若把 api.ts regen（后端 commit）与 fixture 修复（前端 commit）分两个 commit（api.ts 先落），中间那个 commit 的 Frontend CI 必红**（B080: `dd9f703` 红 → `46ba83b` 绿）。

**规约：** 后端加 schema 字段并 regen `api.ts` 后，**立即 `grep` 该 schema 名的前端 fixture**，**同一 commit（或前端先）补齐新字段**，避免 main 上出现瞬时红 Frontend CI。带默认值 ≠ TS optional——契约层 required 由「响应恒含」决定，不由「pydantic 有默认」决定。

**来源：** B080 F004（`benchmark_symbol` / `first_day_caveat` regen 致前端 fixture tsc 红,planner 协调注记#3）。同族 §13（前端契约）+ §27（前端本机绿≠CI 绿）。

## 43. 部署种子数据规则——必须走 alembic data-migration 或显式接入部署链，不能只放 bootstrap CLI（v0.9.55 — B080 F005 沉淀）

**坑：** `workbench-bootstrap` CLI 的幂等 seed（`_import_trials` 27 trials / curated symbol_names）**只手动跑**,不在 `deploy.sh`（只 `alembic upgrade`）/ `workbench-deploy.yml`。→ **部署后生产从不落库,且无调度器自愈**（不同于有 timer 的 `monitoring_metric` 周 job / `reverify` 季 job）。B080 F001 `trial_registry=0` + B079 curated `symbol_name=0` **同源**。对比:OOS 红卡（迁移 0028）/ paper currency（0032）走 **data-migration 随 alembic 自动落地**就没这问题。B080 F005 修复即把 trial 回填改成 data-migration 0033。

**规约：** 凡「**部署后必须存在的种子数据**」,必须走 **alembic data-migration**（随部署自动落地）或**显式接入部署链**,**不能只放 bootstrap CLI**——否则生产静默缺数据、无告警、不自愈。planner 起草含种子数据的 spec 时,acceptance 必须写明「落地路径 = 自动部署（migration / 部署链）,非手动 CLI」（见 planner.md）;generator 加种子数据时同此。

**来源：** B080 F005（trial 回填改 data-migration 0033）。**遗留：** curated `symbol_name` 生产=0 仍未修（B079 done + akshare_spot 日刷 5203 行覆盖显示,非阻断）——可 backlog 治本（bootstrap 接部署链）。同族 §12.10（deploy-artifact 自包含）+ evaluator.md §28（验收指名生产实际读的源）。

## 44. 回测引擎执行限制 = loop-level freeze/restore,勿在 rebalance 内改权重（v0.9.55 — B081 F002/F003 沉淀）

**规律：** 回测引擎里「某名当日不可交易」（停牌禁买卖 / 涨停禁买 / 跌停禁卖 / 退市前停牌窗口）的实现,**不要**在 `_execute_open` 内部改目标权重——会撞上 cost-reservation 缩股 churn + 归一化难题（同 F001#3 band×partial 三次失败）。

**干净方案 = loop-level freeze/restore：** 执行前把受限持仓名（**exact shares + entry/peak**）**冻结取出**并从 target 剔除 → 非受限账本在剩余 tradeable pool 内正常 rebalance（full / partial / lot_rounding 全不动）→ 执行后把冻结名**原样放回**。**equity 守恒**（冻结价值在放回的 shares 里）。F002 停牌 + F003 涨跌停复用同一 `restricted_today` 集合（∪ 合并）,一处逻辑覆盖两类限制。开关 off → 集合空 → **bit 级旧口径**。

**规约：** 凡「部分持仓本轮不可动、其余照常调仓」的需求,**shares-preserving freeze/restore 比「重定位到 current weight」更干净**（后者被 cost 预留缩股）。

**来源：** B081 F002（停牌）+ F003（涨跌停,复用 `restricted_today` ∪）。同族 §28（停牌 ffill/NaN 安全读价）。

## 45. 慢真机回测抗 background-kill 基建——resumable + pickle 缓存 prices（v0.9.55 — B081 F004 沉淀）

**坑：** 8 组 × ~5min 全宇宙回测（236M 数据）在本 harness 下 background task ~20min **必被 kill**,且每次重跑 CSV load ~5min **吃光窗口 → 永不前进**。

**方案：** runner 做 **resumable**（每组结果落 JSON,重跑跳已算组）+ **pickle 缓存 prices**（`to_pickle` / `read_pickle`;pyarrow 缺则用 pickle;reload 5min → 30s）。之后每次重跑续算 1–2 组,数轮收敛。缓存文件 **gitignore**（161MB,勿入 git）。通用于任何「单轮超 harness kill 窗口 + 重复固定加载成本」的慢真机跑。

**来源：** B081 F004（全宇宙 A/B 引擎修真 8 组 sweep,resumable + pickle 缓存抗 kill）。配套 README §经验教训「回测保真度：引擎修真 A/B 必须配本金扫描」（同批 F004→F005 更正的另一半）。

## 46. 分页接口拉取纪律——要么分页拉取，要么显式证明未触顶；失效常量撤回留 None（v0.9.56 — B109 F002 沉淀）

**坑：** Tushare 单次 API 调用**静默截断**（`income_vip` 恰好 9000 行 / `namechange` 恰好 10000 行）——不报错、不抛异常、不置标志位，唯一线索是行数恰好是整数页。★截断**非均匀**：被砍记录富集 vintage（`update_flag=0` 占 18.7% vs `flag=1` 仅 5.2%），而那正是修订率/保留率指标的计算依据 → B109 F001 已交付核心数字全部作废（分页重测 2021FY 修订率 0.525%→1.325%、`flag=0` 保留率 69.4%→93.1%；未触顶期次数字完全不变，反证偏差源就是截断）。

**规约：**
1. **凡分页接口，要么分页拉取（短页才是最后一页 + 触顶守卫 + FetchReport 留痕），要么显式证明未触顶，不存在第三种可接受状态**——「调用成功返回」不等于「拿到了全部」。
2. **撤回失效常量时留 `None` 而非留旧值**——一个已知偏低的数比留空更危险，下游会把它当已验证事实引用。

**来源：** B109 F002（修复 `scripts/research/ashare_pit/fetch.py` + 审计 `docs/audits/B109-F002-tushare-silent-truncation-2026-07-20.md`）。与 B108 E01「被规则挡住不等于被验证过」同族。

## 47. 几何年化为裁定口径时算术必须同表并排；符号相反须显式波动率分解（v0.9.56 — B110 F004 沉淀，扩 §37）

**坑：** 几何年化会把「降波」混进「选股」。B110 主口径几何超额 **+0.9606%**（正）而算术超额 **−0.3237%**（负），**符号相反**：Q5 月度均值 1.2346% **低于**基准 1.2616%，但 Q5 月度 σ=6.84% vs 基准 8.16%，复利拖累少 1.28pp——**全部几何超额来自波动率，选股 alpha 为负**（决定性检验：Q5 保均值缩放到基准波动率 → 超额 −0.3492pp）。而降波不需要该因子就能买到。冻结附录 D2 本已要求算术并排披露，但 signoff 的裁定表未并排。

**规约：** first-look 裁定（§37）以几何年化为裁定口径时，**算术必须同表并排**；两者**符号相反时须显式做波动率分解**（均值差 vs 复利拖累差），不得单凭几何进入放行档。

**来源：** B110 F004（用户裁 NO-GO，推翻 evaluator 的 INCONCLUSIVE）。配套 §37（first-look 判据）+ evaluator.md §36（分母筛选逻辑）。

## 48. 交易过滤后必须按「实际执行的卖腿」重验现金可行性（v0.9.56 — B111 F006-3 沉淀）

**坑：** min-trade / drift-band / skipped-name 降级类交易过滤发生后，成本准备金若基于**过滤前**目标册计算，被跳过的卖单 proceeds 不到账，买单仍按全册 premise 定价 → 超买、现金变负（B111-F006-3 生产实例：Master cash $60.25 → −$18.94）。

**规约：** 任何 trade-filter（min-trade、drift band、skipped-name 降级）落地时，其验收必须含**「跳过卖单不为买单供血」对抗用例**；现金可行性以过滤后的**实际成交**为准重验，不信过滤前的目标册。

**来源：** B111 F006-3（生产负现金事故）。同族 §39（round-trip 成本预留，满仓透支的另一形态）。

## 49. venv/环境自愈探针须覆盖 dev 测试依赖，或自愈走全量 [dev] 同步（v0.9.56 — B111 沉淀）

**坑：** codex-setup.sh 的 venv 自愈只探 `import workbench_api.app`（运行依赖），本地 venv 缺 vcrpy/pytest-recording（测试依赖）时 5 个 VCR 测试**假红**，首次误判成本约 30 分钟。

**规约：** venv 同步探针应**覆盖 dev 测试依赖**（如 `import vcr`），或自愈直接走 `pip install -e backend[dev]` 全量同步——只探运行依赖会让「测试依赖缺失」的假红被误判为产品回归。

**来源：** B111（vcrpy 假红误判 30 分钟）。同族 §26.3（venv copy-install 须重装）+ evaluator.md §34（CI 红物理关联诊断）。

## 50. 「提交前可运行」升级为「HEAD 可从原始输入复跑」（v0.9.57 — B112 沉淀）

**坑：** 本地工作树的修补未提交，验收方在 HEAD 复跑即死——B112-F001-2 的日期 dtype 修复只存在于本地工作树，HEAD 复跑直接失败。

**规约：** 交付含 runner 的产物前，必须在**干净 stash 状态**下从原始输入完整冒烟一遍；「本地能跑」不构成交付条件，「HEAD 能从原始输入复跑」才算。

**来源：** B112-F001-2。同族 §41（子集绿 ≠ 全绿——工作树绿 ≠ HEAD 绿）。

## 51. 数据集拼接防方法论并集污染——同源优先 + 异源补新块（v0.9.57 — B112 沉淀）

**坑：** 同 as_of 不同来源的宇宙块按 `(as_of, ticker)` 取并集，会造出 1700+ 名伪宇宙（B112 实测）——不同源的入选方法论不同，并集产物不属于任何一种方法论。

**规约：** 数据集拼接规则只能是**「同源优先 + 异源补新块」**：同 as_of 重叠区以单一来源为准，异源仅补充该源独有的新块，不得按键并集混合。

**来源：** B112（宇宙块拼接实测 1700+ 名伪宇宙）。同族 §46（外部数据源完整性不可假设）。
