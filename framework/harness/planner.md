...
### 铁律 9：spec 扩新市场/新数据端点须含「先验该端点可达」任务，勿当兄弟端点镜像（v0.9.45 — B062 F001 沉淀）

spec 扩展新市场 / 新数据源 / 新端点时，**必含一个「先验该端点可达 + 格式 + 真返回」的前置任务**，不得假设「同 lib 的兄弟端点（不同市场/函数）」可达且行为一致。B062 HK provider 把 akshare `stock_hk_hist` 当 A 股端点的港股镜像直接用，但两者命中不同主机（HK 走 `33.push2his.eastmoney.com` 可复现 ReadTimeout）→ prod 查 0700.HK 全坏。**写「按 X 维度处理」类条款前，先核 X 在代码里是否真有行为差异**（§22.1 配套：现实已隐含满足则不造装饰机制，避免 generator.md §17 装饰代码）。**配套：** generator.md §23（Generator 实现侧本地实跑真调用）+ §22.1。

**来源：** B062 F001（HK lookup prod 坏，commit `af57842`）；呼应 B062 planner 自承学习。

### satellite 子策略权重口径：spec acceptance 须标注 cap 参数 total-level vs sleeve-relative（v0.9.45 — BL-B011-S2 F002 沉淀）

master sleeve 子策略 `generate_signal().weights_dict()` 必须返回 **sleeve-relative 权重求和=1.0**（`master_portfolio._resolve_child_weights` 直接当 sleeve-relative 用，再按 `planning_weight` 缩放）。故策略说明书里的 **total-portfolio caps**（如 HK-China per-ETF ≤10% total）实施时须换算为 sleeve-relative（÷planning_weight）：sleeve 占 10% 时 per-ETF total 10% = sleeve-relative 1.0（单标的可占满 sleeve）。**坑：** planner 把 total-level cap 值（`max_position_weight=0.10`）直接写进 feature acceptance 当策略参数，与 sleeve-relative sum=1.0 不兼容（`top_n × 0.10 < 1.0` 永远填不满）。**规约：** 写 satellite 策略 acceptance 时，cap 参数须明确标注 **total-level vs sleeve-relative**，total cap 由 master `planning_weight` 承担，避免 generator 二义。
...
### 1.5 复核 signoff 的 FULL PASS 是否名副其实（决策级/真数据批次强制）（v0.9.45 — B061/B062/B063 三例沉淀）

接手 done 前，**复核 evaluator 的 "FULL PASS"/"DONE" 是否名副其实**——尤其是 **core acceptance 为「真实数据 / 真机行为 / 决策依据数字」的批次**：

1. **「框架就绪」/「ready for execution」/「后续执行路径」/「数据源存在」/「部署存在」= 红旗措辞**：见此立即怀疑核心交付（真跑结果）没产出，**降级**该 acceptance 项 + 设硬闸 Soft-watch。
2. **决策点批次**（产出 go/no-go 的批次）：signoff 必须有**真跑数字 + 明确 go/no-go**（见 evaluator.md §29 实测证据硬段），否则不接受 done——B063 即此处置：planner 否决 Codex「FULL PASS→DONE」（核心对比报告从没产出），路由 Generator VM 真跑出真数字才闭合。
3. **evaluator 缺真机 auth / 真数据手段时**：把决策级 / 真数据执行**路由给能可靠 SSH + 跑的 Generator**，或下沉 CI（golden 数据，关联 `docs/dev/test-automation-roadmap.md`）。
4. 发现高估即据实降级 + Soft-watch 硬闸（B061/B062 已做），不替 evaluator 掩盖。

**配套：** evaluator.md §25.1（验收纪律强化）+ §29（signoff 实测证据硬段）。
...
## spec 冻结输入条款落笔前必须核实全窗口可得性；预设「不可得 → 数据缺口」分支（v0.9.57 — B112 沉淀）

**背景：** B112 spec 冻结输入写「top~1500 去偏 PIT 宇宙」，但该宇宙在 2019-2026 全窗口本不存在（baostock 无 zz1000/800 dated 成分、生产宇宙 2021-09 才起且有偏差），直到验收才暴露 → 白烧一轮 fixing。

**规约：**

1. spec 冻结输入条款落笔前必须**核实全窗口可得性**——逐窗口探针验证，非只验当前时点可得。
2. 对可得性存疑的输入，条款应**预设分支「不可得 → 按数据缺口处理」**（降级口径 / 缩小窗口 / 预写的裁定路径），避免 fixing 轮才现场裁定。

来源：B112（top~1500 宇宙全窗口不可得，验收才暴露）。配套上节「Spec 起草必含数据准备步骤」。
...
## Cloud-deploy spec checklist（v0.9.24 — B021 done wrap-up 沉淀）

**触发：** B021 Cloud Deploy & Auth 起 spec 时只覆盖 incremental redeploy 流（CI push → SCP → systemctl restart），**没把"第一次把基础设施安到一台空 VM 上"作为 feature**。结果 Codex L2 reverify 时 VM 上 zero 安装，OAuth/DB/服务全没有，最后 Planner 在 user 授权下用 5 个 commit 7 轮 retry 才把 bootstrap 跑通。这 ~2-3 小时的"踩坑"完全是 spec 时多写 1 个 feature 可以预防的。

**规律：** 任何"把工件部署到一台你之前没用过这种工件的机器"的批次 spec 必须显式含一个 First-time bootstrap feature，覆盖：

| 必做项 | 例子（B021 等价物） |
|---|---|
| 系统层依赖 apt 安装（Python venv、cloud CLI、sqlite3、image runtime 等） | `python3.11`、`python3.11-venv`、`google-cloud-cli`（apt 而非 snap，systemd 友好）、`sqlite3` |
| 目录创建 + 权限（traversal bit + chmod + chown） | `/srv/<app>/releases`、`/opt/<app>/.venv`、`/var/log/<app>`、`/etc/<app>` 755（让 app user 能 traverse 读 config） |
| systemd 单元文件 install + daemon-reload + enable（**不止 restart**） | `workbench-{backend,frontend,backup}.service` + `*.timer` |
| 反代 vhost + 证书签发（cert non-interactive webroot 模式） | nginx vhost + `certbot certonly --webroot` |
| Env 文件初始化（通过 GitHub Secrets 工作流，绝不走聊天/CLI） | `bootstrap-env.yml` workflow_dispatch + `sudo install -m 640 root:<app-group>` |
| 第一次 deploy（触发 CI chain 或手工 trigger workflow_dispatch） | push commit to workbench/backend/ + watch workflow_run 链 |
| 验证手动 prereq（VM SA scope、DNS A、域名 cert）已先完成 | runbook 链接 + 状态表 |

**CI 工作流 spec checklist 配套：** 任何 `workflow_run`-触发的 deploy workflow 都需要在 GitHub Secrets 准备清单中含：

- `DEPLOY_HOST` / `DEPLOY_USER` / `DEPLOY_SSH_PRIVATE_KEY`（基础 3 件）
- **`DEPLOY_SSH_KNOWN_HOSTS`** — ssh-keyscan 产出，否则 SCP/SSH 步骤 `Host key verification failed` 失败（B021 prep #5 漏的就是这条）
- 任何 app-specific secret（OAuth / NextAuth / DB password 等）

**反面案例（B021 F006 fix-round 1-7）：** L2 阻塞 4 次都是 bootstrap 缺项 — `Permission denied (publickey)`（DEPLOY_SSH_KNOWN_HOSTS 缺）→ `No module named alembic`（pip install 缺）→ `password required`（systemctl restart 写两个 service 名 sudoers 不允）→ `does not exist` config（/etc/<app>/ 750 root:root deploy 进不了）→ snap-confine cap_dac_override → frontend hardcoded localhost。每个都是「spec 一句话」可避免的。

### v0.9.25 扩展（B022 done wrap-up 沉淀）

**(c) Production release tarball 必须 ship 业务 page 渲染所需的 docs 类静态资源。** B022 F014 fix-round 1 blocker #1 阻塞是因 `workbench-deploy.yml` rsync 只含 `workbench/backend/` + `workbench/frontend/`，没含 `docs/test-reports/`，但 `/reports` 页运行时 `WORKBENCH_REPORTS_DIR` 默认值指向 release dir 下的 docs 路径 → 文件不存在 → 页面空 → 通过 L1 但 L2 红。规约：**deploy workflow stage step 必须 explicit rsync 业务 page 引用的所有 docs 类目录**（rsync `docs/test-reports` + `docs/specs` 是 B022 实际 fix）；spec checklist 加一项 "List 运行时 page 引用的非 build artifact 静态资源，逐一 ship"。

**(d) L2 acceptance 必须含真实读表 + 真实写表路径（不只 health + auth probe）。** B021 health probe 全绿，B022 reverify 才在 Backlog/Snapshots 真实业务路径发现 `no such table` —— schema drift 不会在 health 暴露。规约：**任何 SQLite/Alembic cloud 批次 L2 至少覆盖 1 个真实读表 + 1 个真实写表 endpoint**，断言返回 200 + payload schema 正确。Recommendations / Backlog CRUD 类 endpoint 都符合。从 spec 起草角度：F014 类 Codex L2 checklist 必须显式列「读表路径 X」+「写表路径 Y」，不能只是 health + login。

**来源：** B021 F006 fix-round 1-7（commits `e5020ea` 起到 `4eb9c48` 止）+ B022 F014 fix-round 1 blocker #1 + B022 signoff §Framework Learnings 新坑（B021 health 全绿但 B022 schema drift 在业务路径才暴露）。配套 generator.md §12.5/12.6 是 Generator 实施层。

### v0.9.27 扩展（B025 done wrap-up 沉淀）

**(e) Cloud-deploy 批次的 deploy workflow 必须含 `workflow_dispatch` trigger + spec acceptance 包含 chore-only deploy race 处理。** B025 F006 round-3 / round-4 两轮被 `Production HEAD ≡ main HEAD` 阻塞，根因不是产品代码漂移，而是 fixing → reverifying → done 的状态机本身推 chore commit（progress.json / features.json / .auto-memory/** 改动），而原 deploy workflow 只接 `workflow_run: ["CI"]` 链触发——状态机文件 paths-ignore 不跑 CI，因此 chore commit 后 production 永远落后一个 SHA。每轮 signoff/blocker commit 后 main 又前进 1 个 chore commit；deploy 又落后。**这是 framework race，不是评估失误。** 规约：(1) deploy workflow 含 `workflow_dispatch` 触发；(2) Generator 推 chore commit（fixing→reverifying / reverifying→done）后若 batch 是 cloud-deployed，紧接一行 `gh workflow run "<App> Deploy" -r main`，并把链接写入 session_notes / handoff；(3) Evaluator 复验 Production/HEAD 时若 diff 仅含状态机文件，按 v0.9.25 §Production/HEAD 等价性 直接接受不同步；若 diff 含产品/spec 文件，Evaluator 自行 dispatch 而非起新 fix-round。spec acceptance 段：cloud-deploy 类批次必须有一条 "deploy workflow 接受 workflow_dispatch + Generator chore commit 后必 dispatch deploy"。配套 `framework/harness/generator.md` §12.7（Generator 实施层）+ `framework/harness/evaluator.md` §21（Evaluator signoff 模板使用层）+ `framework/templates/signoff-report.md` §Post-signoff Deploy（模板段）。

**来源：** B025 F006 round-3 / round-4 deploy drift；commits `afa154d`（`workflow_dispatch` 上线）+ `abaaf6e`（最终签收前 SHA）；signoff `docs/test-reports/B025-us-quality-signoff-2026-05-25.md` §Soft-watch S2 + §Framework Learnings 新规律。

---

## i18n 加新 locale 时 safety regression 必须扩集等价禁词（v0.9.26 — B024 沉淀）

**触发：** B024 是 workbench 首次加 zh-CN locale。原 safety regression `tests/safety/no-execution-buttons.spec.ts` 只扫英文禁词（`execute / place order / send to broker`）；任何中文翻译挑「执行清单 / 立即下单 / 发送券商 / 一键交易」等**英文禁词的中文等价**都能绕过守门。research-only 边界靠中英两语同级 enforced，不能只守一语。

**规约（任何加新 locale 的 spec 起草时硬要求）：**

1. **safety regression spec acceptance 必含「等价禁词扩集」段**，列出新 locale 中所有英文禁词的语义等价词。Generator 同 commit 把新 locale 禁词加入 `tests/safety/no-execution-buttons.spec.ts`（或对应 safety regression 文件），保证扫描器同时检索 JSX 文本节点 + label 属性 + messages bundle 的 key/value。
2. **测试断言数 ≥ 10 spec assertion**，且必须 0 命中。
3. **messages bundle 自身也要扫**——任何 button-类 key 名（heuristic：`button|generate|void|save|submit|...`）后缀的 value 进入扫描器，避免翻译者在 messages bundle 里写禁词但被前端拼装出来。

**中文按钮禁词参考清单（B024 起作为 workbench-trade 项目长期 enforced）：** `执行 / 下单 / 发送券商 / 立即买入 / 实盘 / 真实交易 / 自动交易 / 一键交易`。后续加日韩等其他语种同样规则——本文档列清单的目的不是穷举所有项目，而是**强制每个项目在加新 locale 时显式列等价禁词、写进 spec、写进 safety regression**。

**反面案例：** 若不固化此规则，加新 locale 的 spec 会默认沿用旧 safety regression 不扩集，翻译者在 messages bundle 里写「执行清单」等绕过英文守门——直到 manual code review / 运营线上才发现，那时 prod 已经有"中文版按钮长得像执行入口"的可见 UI。

**来源：** B024-i18n-zh-cn F003 + F006 acceptance；B024 signoff §Framework Learnings 候选 α；commits `57e6132`（F003 实装中文禁词扩集）。

---

## i18n disclaimer / compliance 文案双语永存（v0.9.26 — B024 沉淀）

**触发：** B024 F005 Order ticket Markdown disclaimer 多语化时，spec 初稿一度倾向"按 locale 切换 disclaimer 文本"。最终走双语并存——同一份 Markdown 同时含英中两行 disclaimer，**永远不按 locale cookie 切换内容**。原因：

- Order ticket Markdown 按日期归档（`docs/runs/<date>/order-ticket-<id>.md`），按时间访问的 operator / auditor 不带 locale 上下文，按 locale 生成不同文件会让历史 diff 漂移、单文件丢失另一语言
- legal/compliance / audit 二次审查时希望一文件读完所有语种，避免在多个 locale 文件间跳
- IMMUTABLE 单测 pin 同时锁定英文 + 中文字面值，防"翻译微调"破合规

**规约（任何 disclaimer / 法律免责 / compliance 类静态文案的 spec 必须遵守）：**

1. **双语并存而非按 locale 切换**——同一份产出物（Markdown / PDF cover / email footer / ticket detail）同时含全部支持语种的对应段落，相邻两行或并排列。
2. **每语种字面值各设一个 IMMUTABLE 常量**（如 `DISCLAIMER_LITERAL_EN` + `DISCLAIMER_LITERAL_ZH`），unit test 双语都 pin（任改一字单测红），并各 export 给跨模块复用。
3. **Markdown / PDF 模板的章节标题、表头、checklist item 同样双语并存**（如 `## Trades to place / 待下达交易`，`- [ ] Recorded fills / 在 workbench 记录成交`）。Markdown 永远不按 locale 切换内容——只有 UI 渲染时按 locale 切（但 Markdown 文件本身定格）。
4. **历史 ticket 不动 schema 也不重写**——既有产出物保持原样，只有重新 generate 才走双语模板。

**适用范围扩展：** 不只 disclaimer——任何「compliance / legal / audit / regulatory」类不可按 locale 漂移的文案都适用（如 risk warning、kill-switch trigger 报告、监管报送字段、券商接口往返摘要）。

**反面（已避开）：** 若按 locale 切换 disclaimer，会出现"中文 locale 用户的 ticket 文件没英文 disclaimer / 英文 locale 用户的 ticket 文件没中文 disclaimer"，跨 locale audit 时需要同时拉两份文件比对，且历史归档无法用单一文件视图复审。

**来源：** B024-i18n-zh-cn F005 + F006 acceptance；B024 signoff §Framework Learnings 候选 γ；commits `791c43e`（bilingual Markdown disclaimer + bilingual section titles）。

---

## AI 边界精细化（v0.9.28 — B025 done 沉淀）

**触发：** B025 done 阶段产品规划批次（`docs/product/positioning-2026-05.md` + `user-personas-and-journeys-2026-05.md` approved 2026-05-25）明确 workbench 产品定位为 "AI-augmented personal portfolio decision support tool, built on a quant-strategy chassis"。用户原本对系统期望即 "AI 基于成熟的量化策略结合新闻等信息的分析，给出投资建议"。这与 v0.9.21-v0.9.27 永久边界 `no-AI fit/predict` 一刀切冲突。Planner 在 done 阶段做边界精细化沉淀，让后续 AI advisory 类批次 spec 能精准对齐。

**规约（任何含 AI / LLM 的 spec 必须遵守）：**

把原 `no-AI fit/predict` 一刀切替换为 **5 子条**：

| 子条 | 内容 | 类型 |
|---|---|---|
| (a) | `no-AI auto-execution` — AI 不可触发任何自动下单 / 交易 / 调仓 | **永久禁止** |
| (b) | `no-AI 收益预测数字输出` — AI 不输出"预期年化 X%" / 任何收益预测数字 | **永久禁止** |
| (c) | `no-AI 替代 quant 评分作为唯一决策依据` — AI 是 quant signal 的叠加层，不可跳过 Master Portfolio 评分直接给 buy/sell | **永久禁止** |
| (d) | `AI 输出必须基于 quant signal + real data + 可引用 news` — 无引用的黑盒建议禁止；每条建议必带 quant_signal SHA / news source URL | **强制要求** |
| (e) | AI 做以下事项**允许**：解释（quant signal / 指标 / Sharpe 等术语 tooltip）/ summarize（news / SEC filings）/ translate（zh ↔ en）/ context aggregation / Robinhood-style 简化文案 | **允许** |

**spec acceptance 段落模板：**

任何含 AI / LLM 的批次 spec 必须在 §3 永久硬边界段列出本 5 子条全集（不可省略，不可改写）。AI advisory engine 类批次 acceptance 必须显式：

- 每条 AI 输出含 quant_signal SHA + news source URL 引用（pytest assertion 覆盖：≥3 个无引用输入 → AI 拒绝输出 / 标 `INSUFFICIENT_GROUNDING`）
- 单元测试覆盖"AI 输出含 'predicted return X%' 等收益预测数字 → 输出被 sanitize / 标错"（pytest assertion 覆盖：≥10 红队样本，命中即失败）
- AI 不直接连 broker / kill-switch / 写 manual_execution 表（架构层禁止 import 这些模块）
- LLM provider 选型 + cost 控制 + cache 策略 + safety eval 必须在批次内显式落地（不可推迟到下一批次）

**适用范围：**

- 任何含 LLM API 调用 / AI 输出文案直接呈现给用户的批次
- News / market context aggregation 类批次（即使只 summarize 不 predict 也按本规约 acceptance）
- Robinhood-style 简化批次（若用 AI 改写文案，按 (e) 允许，但需 spec 显式 acceptance 段提及）

**反面案例预防（B025 done 阶段确认）：**

- 用户最初对系统期望 = "AI 给我 actionable buy/sell signal + 预期收益"，路径 C（全软化）的实际风险（金融预测不可靠、AI 偶发幻觉、跟单一次损失数 X% 真实财产）已显式列入 `docs/product/positioning-2026-05.md` §AI 角色与边界
- B025 done 阶段第 6 轮 Q&A 用户选择路径 B 精细版（quant + real data + news → AI 综合建议）而非路径 C
- 本规约就是把这个选择固化为 framework 强制约束，防止后续批次 spec 意图飘移到路径 C

**配套：** `docs/product/positioning-2026-05.md` §1.1 AI 角色与边界（8 可做 / 8 不可做表）+ §6.1 永久边界更新（旧 vs 新表述对照）+ `.auto-memory/project-status.md` §永久硬边界 v0.9.28 AI 边界精细化段。三处需在 v0.9.X 后续沉淀时保持一致（任改一处需同步另两处）。

**来源：** B025 done 阶段产品规划批次 6 轮用户 Q&A；commit `12de7fe`（docs(product): integrate AI-advisor path B into positioning + journeys）+ `5d3a425`（mark positioning + personas as approved）。
