...
SYSTEM_PROMPT = (
    ...
    "\n"
    "OUTPUT LANGUAGE: write the human-facing `explanation` text in Simplified "
    "Chinese (zh-CN). The boundary rules above apply identically to Chinese "
    "output. Keep every `references` token, ticker symbol, date, and numeric "
    "value exactly as given in the input — do NOT translate or alter them.\n"
    "\n"
