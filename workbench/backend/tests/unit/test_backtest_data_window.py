...
def test_cli_main_persists_window(
    ...
    # B062 F002 — 3rd CN/HK loader (reuse offline _FakePrices). B063 F001 — 4th
    # FX loader; a no-op fake (returns no FX points, no key/network).
    class _FakeFx:
        def fetch_fx(self, series_id: str, *, limit: int) -> list[object]:
            return []

    monkeypatch.setattr(
        refresh_cli,
        "_build_loaders",
        lambda: (_FakePrices(), _FakeFundamentals(), _FakePrices(), _FakeFx()),
    )
    rc = refresh_cli.main(
